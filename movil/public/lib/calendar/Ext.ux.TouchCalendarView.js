...
Ext.ux.TouchCalendarView = Ext.extend(Ext.DataView, {
				...
				'<thead>',
					'<tr>',
						'<tpl for="this.getDaysArray(values)">',
							...
							'<p>{[xindex]}<p/>',
							'<p>{[this.getHeaderClass(xindex)]}<p/>',							
								'<tpl if="xindex === 4">',
	...
	onTableHeaderTap: function(e, el){
		el = Ext.fly(el);
		alert(el);
		alert(this.prevPeriodCls);
		alert(this.nextPeriodCls);
		alert(el.hasCls(this.prevPeriodCls));
				...
				'<thead>',
					'<tr>',
						'<tpl for="this.getDaysArray(values)">',
							'<th class="{[this.getHeaderClass(xindex)]}">',													
								'<tpl if="xindex === 4">',
									'<span>{[this.me.currentDate.format("F")]} {[this.me.currentDate.format("d")]}</span>',
								'</tpl>',
								//'{[this.me.currentDate.format("D")]}',
							'</th>',
						'</tpl>',
					'</tr>',
				'</thead>',	
		    '<tbody>',
				'<tpl for=".">',
					'<tr>',
						...
						'<td class="time-block" datetime="{[this.me.getDateAttribute(values.date)]}" colspan="7" >',
