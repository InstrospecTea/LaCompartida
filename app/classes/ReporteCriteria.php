...
class ReporteCriteria {
	 ...
	 * |  |  +-- horas_visibles: Horas que ve el Cliente en nota de liquidaci�n (tras revisi�n)
	 ...
	 * |  |  |  |  \-- horas_por_pagar: Horas Cobradas que a�n no han sido pagadas
	 * |  |  |  +-- horas_por_cobrar: Horas Visibles que a�n no se Emiten al Cliente
	 ...
	 * |  |  |  +-- valor_cobrado: Valor monetario que corresponde a cada Profesional, en una Liquidaci�n ya Emitida
	 * |  |  |  +-- valor_tramites: Valor monetario de tr�mites que corresponde a cada Profesional, en una Liquidaci�n ya Emitida
	 * |  |  |  |  +-- valor_pagado: Valor Cobrado que ha sido Pagado
	 * |  |  |  |  \-- valor_por_pagar: Valor Cobrado que a�n no ha sido pagado
	 ...
	 * +-- valor_trabajado_estandar: Horas Trabajadas por THH Est�ndar, para todo Trabajo
	 * +-- valor_estandar: Valor Cobrado, si se hubiera usado THH Est�ndar
	 * +-- diferencia_valor_estandar: Valor Cobrado - Valor Est�ndar
	 * +-- valor_hora: Valor Cobrado / Horas Cobradas
	 * +-- rentabilidad_base: Valor Cobrado / Valor Trabajado Est�ndar
	 * +-- rentabilidad: Valor Cobrado / Valor Est�ndar
	 ...
	 * @return void s�lo asigna los filtros necesarios seg�n tipo de dato
	 */
	...
	public function cobroQuery() { //Query que a�ade rows para los datos de Cobros emitidos que no cuentan Trabajos
		...
		// todo crear m�todo que identifique cuando ejectuar la query cobro
	...
	public function Query() {
		...
		// Obtiene todos los datos para Tr�mites
	...
	private function rightValueForColumn($key, $type) {
		if (!in_array($key, $this->columns)) {
			return "'La columna {$key} no est� en el arreglo de columnas'";
		}
		$column_value = null;
		...
		switch ($key) {
			...
			case 'id_trabajo':
				...
				break;
			default:
				$column_value = "'La columna {$key} no est� definida'";
