...
class PasswordStrengthChecker {
	...
	public function RateUsingGoogle() {
		$rate = 1;
		try {
			$ch = curl_init('https://www.google.com/accounts/RatePassword?Passwd=' . urlencode($this->password));
	...
	public function RateUsingIndicators() {
		$rate = 1;
    $this->calculateAll();
    $score = $this->passwordInfo['averageScore'];
    if ($score <= 35) {
      $rate = 1;
    } else if ($score <= 55) {
      $rate = 2;
