...
class FacturaPago extends Objeto {
	...
	public static $campos_carga_masiva = array(
		...
		'nro_documento' => 'N�mero Doc Pago',
		'nro_cheque' => 'N�mero Cheque',
	...
	function HtmlListaPagos($sesion, & $factura, $id_documento) {
		...
		$html = "<table width=\"500\">
					<tr bgcolor=\"#aaffaa\" style=\"font-size: 10pt;\">
						...
						<th width=200>" . __('Descripci�n') . "</th>
		...
		for ($i = 0; $i < $lista_pagos->num; $i++) {
			...
			$html .= "<td align=center>
									...
									<img src='" . Conf::ImgDir() . "/cruz_roja_nuevo.gif' onclick=\"if( confirm('Est� eliminando un pago. Se reajustar�n los saldos de los documentos asociados. �Desea continuar?') )EliminarPago('" . $pago->fields['id_factura_pago'] . "');\" />";
	...
	function generarDocumentoPago($parser_factura_pago, $theTag = '', $lang = 'es', $arr_fila_tmp = null) {
		...
		switch ($theTag) {
			...
			case 'DATOS_FACTURA_PAGO':
				...
				$html = str_replace('%detraccion%', __('Detracci�n'), $html);
	...
	function GetSoyElUltimoPago($id_factura_pago = null) {
		...
		//de la lista de facturas asociadas, buscamos a la �ltima factura_pago
	...
	/**
	 * Descarga el reporte excel b�sico seg�n configuraciones
	 */
	...
	public function QueryReporte($orden, $where, $id_concepto, $id_banco, $id_cuenta, $id_estado,
		...
		$grupo_ventas, $razon_social, $descripcion_factura, $codigo_cliente = null) {
		...
		return "SELECT SQL_CALC_FOUND_ROWS
			...
			LEFT JOIN cta_cte_fact_mvto AS ccfm ON fp.id_factura_pago = ccfm.id_factura_pago
	...
	public function DatosReporte($orden, $where, $id_concepto, $id_banco, $id_cuenta, $id_estado,
		...
		//agregar al reporte de factura las columnas, monto real - observaciones - Saldo - fecha �ltimo pago
