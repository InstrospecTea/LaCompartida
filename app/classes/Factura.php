...
class Factura extends Objeto {
	...
	public $arrayanchors=array();
	public $arrayadelantos=array();
	...
	function SaldoAdelantosDisponibles($codigo_cliente, $id_contrato, $pago_honorarios, $pago_gastos, $id_moneda = null, $tipos_cambio = null) {
		...
		$this->arrayadelantos=array();
		$query = "SELECT id_documento, saldo_pago, documento.id_moneda, prm_moneda.tipo_cambio
			FROM documento
		...
		$resp = mysql_query($query, $this->sesion->dbh) or Utiles::errorSQL($query, __FILE__, __LINE__, $this->sesion->dbh);
		$saldo = 0;
		while (list($id_adelanto, $saldo_pago, $moneda_pago, $tipo_cambio) = mysql_fetch_array($resp)) {
			if ($id_moneda) {
				$tipo_cambio = $cambios[$moneda_pago];
			}
			$saldo += -$saldo_pago * $tipo_cambio;
			$this->arrayadelantos[$id_adelanto]=-1*($saldo_pago * $tipo_cambio);
		}
		arsort($this->arrayadelantos);
		if (!$saldo) {
			return '';
		}
		if ($id_moneda) {
	...
	function GenerarDocumento($parser_factura, $theTag = '', $lang = 'es', $xml = false) {
		...
		switch ($theTag) {
			...
			case 'ENCABEZADO':
				...
				$html2 = $this->ReemplazarAnchor($theTag,'%linea1%', $PdfLinea1, $html2);
				$html2 = $this->ReemplazarAnchor($theTag,'%linea2%', $PdfLinea2, $html2);
				$html2 = $this->ReemplazarAnchor($theTag,'%linea3%', $PdfLinea3, $html2);
				$html2 = $this->ReemplazarAnchor($theTag,'%ciudad%', $CiudadSignatura, $html2);
				$html2 = $this->ReemplazarAnchor($theTag,'%LogoDoc%', $logo_doc, $html2);
				...
				if ($lang == 'es') {
					$html2 = $this->ReemplazarAnchor($theTag,'%numero_factura%', __($glosa_tipo_doc) . ' No. ' . $numero_factura, $html2);
					$html2 = $this->ReemplazarAnchor($theTag,'%Senores%', 'SEÑORES', $html2);
					$html2 = $this->ReemplazarAnchor($theTag,'%tipo_doc_legal%', $glosa_tipo_doc_mayus, $html2);
					$html2 = $this->ReemplazarAnchor($theTag,'%numero_doc_legal%', $numero_factura, $html2);
					$html2 = $this->ReemplazarAnchor($theTag,'%numero_factura_cyc%', __('FACTURA DE VENTA NO.').' '. $numero_factura, $html2);
				} else {
					$titulos_es = array('Sr.', 'Sra.', 'Srta.');
					$titulos_en = array('Mr.', 'Mrs.', 'Ms.');
					$titulo_contacto = str_replace($titulos_es, $titulos_en, $titulo_contacto);
					$html2 = $this->ReemplazarAnchor($theTag,'%numero_factura%', __($glosa_tipo_doc) . ' No. ' . $numero_factura, $html2);
					$html2 = $this->ReemplazarAnchor($theTag,'%Senores%', 'Messrs', $html2);
					$html2 = $this->ReemplazarAnchor($theTag,'%numero_factura_cyc%', __('INVOICE').' '. $numero_factura, $html2);
				}
				$html2 =  $this->ReemplazarAnchor('ENCABEZADO','%subtitulo%', '', $html2);

				if (method_exists('Conf', 'Server') && method_exists('Conf', 'ImgDir')) {
					$html2 = $this->ReemplazarAnchor($theTag,'%logo_cobro%', Conf::Server() . Conf::ImgDir(), $html2);
				}
				...
				$html2 = $this->ReemplazarAnchor($theTag,'%nombre_cliente%', $this->fields['cliente'], $html2);
				$html2 = $this->ReemplazarAnchor($theTag,'%glosa_cliente%', $glosa_cliente, $html2);
				$html2 = $this->ReemplazarAnchor($theTag,'%glosa_cliente_mayuscula%', strtoupper($glosa_cliente), $html2);
				$html2 = $this->ReemplazarAnchor($theTag,'%encargado_comercial%', $encargado_comercial, $html2);
				$html2 = $this->ReemplazarAnchor($theTag,'%rut_cliente%', $this->fields['RUT_cliente'], $html2);
				...
				$html2 = $this->ReemplazarAnchor($theTag,'%nombre_encargado%', strtoupper($titulo_contacto . ' ' . $contacto . ' ' . $apellido_contacto), $html2);
				$html2 = $this->ReemplazarAnchor($theTag,'%direccion_cliente%', $direccion_cliente, $html2);
				$html2 = $this->ReemplazarAnchor($theTag,'%direccion_cliente_mayuscula%', strtoupper($direccion_cliente), $html2);
				$html2 = $this->ReemplazarAnchor($theTag,'%comuna_cliente%', $comuna_cliente, $html2);
				$html2 = $this->ReemplazarAnchor($theTag,'%ciudad_cliente%', $ciudad_cliente, $html2);
				$html2 = $this->ReemplazarAnchor($theTag,'%giro_cliente%', $giro_cliente, $html2);
				$html2 = $this->ReemplazarAnchor($theTag,'%lugar_facturacion%', UtilesApp::GetConf($this->sesion, 'LugarFacturacion'), $html2);
				$html2 = $this->ReemplazarAnchor($theTag,'%num_dia%', date('d', strtotime($fecha_factura)), $html2);
				$html2 = $this->ReemplazarAnchor($theTag,'%glosa_mes%', str_replace($meses_org, $mes_largo_es, date('M', strtotime($fecha_factura))), $html2);
				$html2 = $this->ReemplazarAnchor($theTag,'%num_anio%', date('Y', strtotime($fecha_factura)), $html2);
				$html2 = $this->ReemplazarAnchor($theTag,'%num_mes%', date('m', strtotime($fecha_factura)), $html2);
				$html2 = $this->ReemplazarAnchor($theTag,'%num_anio_2cifras%', date('y', strtotime($fecha_factura)), $html2);
				$html2 = $this->ReemplazarAnchor($theTag,'%num_anio_dos_cifras%', date('y', strtotime($fecha_factura)), $html2);

				$anio_yyyy = date('Y', strtotime($fecha_factura));
				$html2 = $this->ReemplazarAnchor($theTag,'%num_anio_ultimacifra%', $anio_yyyy[3], $html2);
				if ($lang == 'es') {
					$html2 = $this->ReemplazarAnchor($theTag,'%fecha_actual%', str_replace($meses_org, $mes_corto, date('j-M-y', strtotime($fecha_factura))), $html2);
					$html2 = $this->ReemplazarAnchor($theTag,'%glosa_fecha%', 'FECHA', $html2);
					$html2 = $this->ReemplazarAnchor($theTag,'%ATN%', 'ATENCION', $html2);
					$html2 = $this->ReemplazarAnchor($theTag,'%id_cobro%', '  ' . $id_cobro, $html2);
				} elseif ($lang == 'en') {
					$html2 = $this->ReemplazarAnchor($theTag,'%fecha_actual%', str_replace($meses_org, $month_short, date('M-d-y', strtotime($fecha_factura))), $html2);
					$html2 = $this->ReemplazarAnchor($theTag,'%glosa_fecha%', 'DATE', $html2);
					$html2 = $this->ReemplazarAnchor($theTag,'%ATN%', 'Attention', $html2);
					$html2 = $this->ReemplazarAnchor($theTag,'%id_cobro%', '   <br> INVOICE No.   ' . $id_cobro, $html2);
				}
				break;
			...
			case 'DATOS_FACTURA':
				...
				$resp = mysql_query($query, $this->sesion->dbh) or Utiles::errorSQL($query, __FILE__, __LINE__, $this->sesion->dbh);
				$respuesta=mysql_fetch_array($resp);
				if (UtilesApp::GetConf($this->sesion, 'NuevoModuloFactura')) {
					list($factura_id_moneda, $factura_descripcion, $id_cobro, $cobro_id_moneda, $fecha_ini, $fecha_fin, $porcentaje_impuesto, $glosa_moneda, $glosa_moneda_plural, $simbolo, $cifras_decimales, $monto_subtotal, $monto_subtotal_sin_descuento, $descuento_honorarios, $honorarios, $subtotal_gastos, $monto_gastos, $impuesto, $total, $descripcion_subtotal_gastos, $descripcion_subtotal_gastos_sin_impuesto, $subtotal_gastos_con_impuesto, $subtotal_gastos_sin_impuesto) = $respuesta;
				} else {
					list($factura_id_moneda, $factura_descripcion, $id_cobro, $cobro_id_moneda, $fecha_ini, $fecha_fin, $porcentaje_impuesto, $glosa_moneda, $glosa_moneda_plural, $simbolo, $cifras_decimales, $monto_subtotal, $monto_subtotal_sin_descuento, $descuento_honorarios, $honorarios, $subtotal_gastos, $monto_gastos, $impuesto, $total) = $respuesta;
				}
				...
				if ($descuento_honorarios > 0) {
					$html2 = $this->ReemplazarAnchor($theTag,'%tr_descuento%', '<tr>
					...
					$html2 = $this->ReemplazarAnchor($theTag,'%tr_descuento%', '', $html2);
				}

				$html2 = $this->ReemplazarAnchor($theTag,'%porcentaje_impuesto_sin_simbolo%', (int) ($porcentaje_impuesto), $html2);
				...
				if ($mostrar_honorarios) {
					$html2 = $this->ReemplazarAnchor($theTag,'%honorarios_periodo%', $honorarios_descripcion, $html2);
				} else {
					$html2 = $this->ReemplazarAnchor($theTag,'%honorarios_periodo%', '', $html2);
				}
				if ($subtotal_gastos_con_impuesto) {
					$html2 = $this->ReemplazarAnchor($theTag,'%gastos_con_impuesto_periodo%', $descripcion_subtotal_gastos, $html2);
				} else {
					$html2 = $this->ReemplazarAnchor($theTag,'%gastos_con_impuesto_periodo%', '', $html2);
				}
				if ($mostrar_gastos_sin_impuesto) {
					$html2 = $this->ReemplazarAnchor($theTag,'%gastos_sin_impuesto_periodo%', $descripcion_subtotal_gastos_sin_impuesto, $html2);
				} else {
					$html2 = $this->ReemplazarAnchor($theTag,'%gastos_sin_impuesto_periodo%', '', $html2);
				}
				if ($lang == 'es') {
					if ($descuento_honorarios > 0) {
						$html2 = $this->ReemplazarAnchor($theTag,'%<br><br>%', '<br><br>', $html2);
					} else {
						$html2 = $this->ReemplazarAnchor($theTag,'%<br><br>%', '<br><br><br><br>', $html2);
					}
					if ($mostrar_honorarios) {
						if (UtilesApp::GetConf($this->sesion, 'UsarGlosaFacturaMayusculas')) {
							$html2 = $this->ReemplazarAnchor($theTag,'%servicios_periodo%', strtoupper($factura_descripcion_separado), $html2);
							$html2 = $this->ReemplazarAnchor($theTag,'%servicios_periodo%', strtoupper('Honorarios por servicios profesionales prestados %fecha_ini% %fecha_fin%'), $html2);
						} else {
							$html2 = $this->ReemplazarAnchor($theTag,'%servicios_periodo%', $factura_descripcion_separado, $html2);
							$html2 = $this->ReemplazarAnchor($theTag,'%servicios_periodo%', 'Honorarios por servicios profesionales prestados %fecha_ini% %fecha_fin%', $html2);
						}
					} else {
						$html2 = $this->ReemplazarAnchor($theTag,'%servicios_periodo%', '', $html2);
						$html2 = $this->ReemplazarAnchor($theTag,'%servicios_periodo%', '', $html2);
					}
					$meses_org = array('Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec');
					$mes_corto = array('jan', 'feb', 'mar', 'abr', 'may', 'jun', 'jul', 'ago', 'sep', 'oct', 'nov', 'dic');
					if ($fecha_ini && $fecha_ini != '0000-00-00') {
						$html2 = $this->ReemplazarAnchor($theTag,'%fecha_ini%', 'desde ' . str_replace($meses_org, $mes_corto, date('j-M-y', strtotime($fecha_ini))), $html2);
					} else {
						$html2 = $this->ReemplazarAnchor($theTag,'%fecha_ini%', '', $html2);
					}
					if ($fecha_fin && $fecha_fin != '0000-00-00') {
						$html2 = $this->ReemplazarAnchor($theTag,'%fecha_fin%', 'hasta ' . str_replace($meses_org, $mes_corto, date('j-M-y', strtotime($fecha_fin))), $html2);
					} else {
						$html2 = $this->ReemplazarAnchor($theTag,'%fecha_fin%', '', $html2);
					}
					if ($mostrar_honorarios) {
						$html2 = $this->ReemplazarAnchor($theTag,'%texto_honorarios%', 'HONORARIOS', $html2);
						$html2 = $this->ReemplazarAnchor($theTag,'%glosa_honorarios%', $factura_descripcion_separado, $html2);
					} else {
						$html2 = $this->ReemplazarAnchor($theTag,'%texto_honorarios%', '', $html2);
						$html2 = $this->ReemplazarAnchor($theTag,'%glosa_honorarios%', '', $html2);
					}

					$html2 = $this->ReemplazarAnchor($theTag,'%texto_gastos%', 'GASTOS', $html2);
					$html2 = $this->ReemplazarAnchor($theTag,'%texto_descripcion_gastos%', 'Gastos incurridos en su caso, según relación adjunta.', $html2);
					$html2 = $this->ReemplazarAnchor($theTag,'%total_honorarios_y_gastos%', 'Total servicios profesionales y gastos incurridos', $html2);
					$html2 = $this->ReemplazarAnchor($theTag,'%pje_impuesto%', $porcentaje_impuesto . '%', $html2);
					if (UtilesApp::GetConf($this->sesion, 'UsarImpuestoSeparado')) {
						if (method_exists('Conf', 'GetConf')) {
							$html2 = $this->ReemplazarAnchor($theTag,'%texto_impuesto%', __('IVA') . ' (' . Conf::GetConf($this->sesion, 'ValorImpuesto') . '%)', $html2);
						} else if (method_exists('Conf', 'ValorImpuesto')) {
							$html2 = $this->ReemplazarAnchor($theTag,'%texto_impuesto%', __('IVA') . ' (' . $porcentaje_impuesto . '%)', $html2);
						}
					} else {
						$html2 = $this->ReemplazarAnchor($theTag,'%texto_impuesto%', '', $html2);
					}
					$html2 = $this->ReemplazarAnchor($theTag,'%descripcion%', '', $html2);
					$html2 = $this->ReemplazarAnchor($theTag,'%texto_total%', 'Total ', $html2);
					$html2 = $this->ReemplazarAnchor($theTag,'%firma%', 'Firma', $html2);
					if ($descuento_honorarios > 0) {
						$html2 = $this->ReemplazarAnchor($theTag,'%descuento_honorarios%', '- ' . number_format($descuento_honorarios, $cifras_decimales, $idioma->fields['separador_decimales'], $idioma->fields['separador_miles']), $html2);
						$html2 = $this->ReemplazarAnchor($theTag,'%descuento_glosa%', __('Descuento'), $html2);
					} else {
						$html2 = $this->ReemplazarAnchor($theTag,'%descuento_honorarios%', '', $html2);
						$html2 = $this->ReemplazarAnchor($theTag,'%descuento_glosa%', '', $html2);
					}
				} else if ($lang == 'en') {
					$html2 = $this->ReemplazarAnchor($theTag,'%firma%', 'Signature', $html2);
					if ($descuento_honorarios > 0) {
						$html2 = $this->ReemplazarAnchor($theTag,'%<br><br>%', '', $html2);
					} else {
						$html2 = $this->ReemplazarAnchor($theTag,'%<br><br>%', '<br><br>', $html2);
					}
					$html2 = $this->ReemplazarAnchor($theTag,'%servicios_periodo%', $factura_descripcion_separado, $html2);
					$html2 = $this->ReemplazarAnchor($theTag,'%servicios_periodo%', 'For legal services rendered %fecha_ini% %fecha_fin%', $html2);
					$meses_org = array('Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec');
					$month_short = array('jan', 'feb', 'mar', 'apr', 'may', 'jun', 'jul', 'aug', 'sep', 'oct', 'nov', 'dec');
					if ($fecha_ini && $fecha_ini != '0000-00-00') {
						$html2 = $this->ReemplazarAnchor($theTag,'%fecha_ini%', 'from ' . str_replace($meses_org, $month_short, date('M-d-y', strtotime($fecha_ini))), $html2);
					} else {
						$html2 = $this->ReemplazarAnchor($theTag,'%fecha_ini%', '', $html2);
					}
					if ($fecha_fin && $fecha_fin != '0000-00-00') {
						$html2 = $this->ReemplazarAnchor($theTag,'%fecha_fin%', 'until ' . str_replace($meses_org, $month_short, date('M-d-y', strtotime($fecha_fin))), $html2);
					} else {
						$html2 = $this->ReemplazarAnchor($theTag,'%fecha_fin%', '', $html2);
					}

					if ($mostrar_honorarios) {
						$html2 = $this->ReemplazarAnchor($theTag,'%texto_honorarios%', 'LEGAL SERVICES', $html2);
						$html2 = $this->ReemplazarAnchor($theTag,'%glosa_honorarios%', $factura_descripcion_separado, $html2);
					} else {
						$html2 = $this->ReemplazarAnchor($theTag,'%texto_honorarios%', '', $html2);
						$html2 = $this->ReemplazarAnchor($theTag,'%glosa_honorarios%', '', $html2);
					...
					$html2 = $this->ReemplazarAnchor($theTag,'%texto_gastos%', 'EXPENSES', $html2);
					$html2 = $this->ReemplazarAnchor($theTag,'%texto_descripcion_gastos%', 'Expenses incurred in this case.', $html2);
					$html2 = $this->ReemplazarAnchor($theTag,'%total_honorarios_y_gastos%', 'Total legal services and expenses', $html2);
					if (UtilesApp::GetConf($this->sesion, 'UsarImpuestoSeparado')) {
						$html2 = $this->ReemplazarAnchor($theTag,'%texto_impuesto%', __('IVA') . ' (' . UtilesApp::GetConf($this->sesion, 'ValorImpuesto') . '%)', $html2);
					} else {
						$html2 = $this->ReemplazarAnchor($theTag,'%texto_impuesto%', '', $html2);
					}
					$html2 = $this->ReemplazarAnchor($theTag,'%descripcion%', '', $html2);
					if ($lang == 'en') {
						...
						$html2 = $this->ReemplazarAnchor($theTag,'Honorarios por asesorías prestadas', 'Legal services rendered', $html2);
						$html2 = $this->ReemplazarAnchor($theTag,'durante el mes de', 'during the month of', $html2);
					...
					if ($descuento_honorarios > 0) {
						$html2 = $this->ReemplazarAnchor($theTag,'%descuento_honorarios%', '- ' . number_format($descuento_honorarios, $cifras_decimales, $idioma->fields['separador_decimales'], $idioma->fields['separador_miles']), $html2);
						$html2 = $this->ReemplazarAnchor($theTag,'%descuento_glosa%', __('Discount'), $html2);
					} else {
						$html2 = $this->ReemplazarAnchor($theTag,'%descuento_honorarios%', '', $html2);
						$html2 = $this->ReemplazarAnchor($theTag,'%descuento_glosa%', '', $html2);
					}
				...
				if (UtilesApp::GetConf($this->sesion, "DescripcionFacturaConAsuntos")) {
					...
					$html2 = $this->ReemplazarAnchor($theTag,'%factura_descripcion%', $factura_descripcion_con_asuntos, $html2);
					$html2 = $this->ReemplazarAnchor($theTag,'%asuntos%', "", $html2);
					$html2 = $this->ReemplazarAnchor($theTag,'%cod_asuntos%', "", $html2);
				} else {
					$html2 = $this->ReemplazarAnchor($theTag,'%asuntos%', $asuntos, $html2);
					$html2 = $this->ReemplazarAnchor($theTag,'%cod_asuntos%', $cod_asuntos, $html2);
				}
				if (method_exists('Conf', 'Server') && method_exists('Conf', 'ImgDir')) {
					$html2 = $this->ReemplazarAnchor($theTag,'%logo_cobro%', Conf::Server() . Conf::ImgDir(), $html2);
				}
				$html2 = $this->ReemplazarAnchor($theTag,'%glosa_moneda_factura%', '%' . $cobro_moneda->moneda[$factura_id_moneda]['glosa_moneda'] . '%', $html2);
				$html2 = $this->ReemplazarAnchor($theTag,'%Peso%', 'PESOS', $html2);
				if ($lang == 'es') {
					$html2 = $this->ReemplazarAnchor($theTag,'%Dólar%', 'DÓLARES', $html2);
				} else {
					$html2 = $this->ReemplazarAnchor($theTag,'%Dólar%', 'DOLLARS', $html2);
				}

				$html2 = $this->ReemplazarAnchor($theTag,'%Euro%', 'EUROS', $html2);
				if (method_exists('Conf', 'GetConf')) {
					$html2 = $this->ReemplazarAnchor($theTag,'%porcentaje%', Conf::GetConf($this->sesion, 'ValorImpuesto') . '%', $html2);
				} else if (method_exists('Conf', 'ValorImpuesto')) {
					$html2 = $this->ReemplazarAnchor($theTag,'%porcentaje%', Conf::ValorImpuesto() . '%', $html2);
				}
				...
				if (UtilesApp::GetConf($this->sesion, 'CalculacionCyC')) {
					...
					if ($mostrar_honorarios) {
						$html2 = $this->ReemplazarAnchor($theTag,'%monto_honorarios%', number_format($monto_subtotal, $moneda_factura->fields['cifras_decimales'], $idioma->fields['separador_decimales'], $idioma->fields['separador_miles']), $html2);
					} else {
						$html2 = $this->ReemplazarAnchor($theTag,'%monto_honorarios%', '', $html2);
					}
					if ($mostrar_gastos_con_impuesto) {
						$html2 = $this->ReemplazarAnchor($theTag,'%monto_gastos%', number_format($subtotal_gastos, $moneda_factura->fields['cifras_decimales'], $idioma->fields['separador_decimales'], $idioma->fields['separador_miles']), $html2);
					} else {
						$html2 = $this->ReemplazarAnchor($theTag,'%monto_gastos%', '', $html2);
					}
					$total_factura_netto = round($monto_subtotal_sin_descuento + $subtotal_gastos, $moneda_factura->fields['cifras_decimales']);
					$html2 = $this->ReemplazarAnchor($theTag,'%monto_total%', number_format($total_factura_netto, $moneda_factura->fields['cifras_decimales'], $idioma->fields['separador_decimales'], $idioma->fields['separador_miles']), $html2);
					...
					if (( ( method_exists('Conf', 'GetConf') && Conf::GetConf($this->sesion, 'UsarImpuestoSeparado') ) || ( method_exists('Conf', 'UsarImpuestoSeparado') && Conf::UsarImpuestoSeparado() ))) {
						$html2 = $this->ReemplazarAnchor($theTag,'%monto_impuestos%', number_format($impuesto_factura, $moneda_factura->fields['cifras_decimales'], $idioma->fields['separador_decimales'], $idioma->fields['separador_miles']), $html2);
					} else {
						$html2 = $this->ReemplazarAnchor($theTag,'%monto_impuestos%', '', $html2);
					}
					if (( ( method_exists('Conf', 'GetConf') && Conf::GetConf($this->sesion, 'UsarImpuestoSeparado') ) || ( method_exists('Conf', 'UsarImpuestoSeparado') && Conf::UsarImpuestoSeparado() ))) {
						$html2 = $this->ReemplazarAnchor($theTag,'%monto_total_brutto%', number_format($total_factura_netto + $impuesto_factura, $moneda_factura->fields['cifras_decimales'], $idioma->fields['separador_decimales'], $idioma->fields['separador_miles']), $html2);
					} else {
						$html2 = $this->ReemplazarAnchor($theTag,'%monto_total_brutto%', '', $html2);
					}
				} else {
					if ($mostrar_honorarios) {
						$html2 = $this->ReemplazarAnchor($theTag,'%monto_honorarios%', number_format($monto_subtotal_sin_descuento, $moneda_factura->fields['cifras_decimales'], $idioma->fields['separador_decimales'], $idioma->fields['separador_miles']), $html2);
					} else {
						$html2 = $this->ReemplazarAnchor($theTag,'%monto_honorarios%', '', $html2);
					}
					if ($mostrar_gastos_con_impuesto) {
						$html2 = $this->ReemplazarAnchor($theTag,'%monto_gastos%', number_format($monto_gastos_sin_impuesto, $moneda_factura->fields['cifras_decimales'], $idioma->fields['separador_decimales'], $idioma->fields['separador_miles']), $html2);
					} else {
						$html2 = $this->ReemplazarAnchor($theTag,'%monto_gastos%', '', $html2);
					}

					$html2 = $this->ReemplazarAnchor($theTag,'%monto_total%', number_format($monto_subtotal + $monto_gastos_sin_impuesto, $moneda_factura->fields['cifras_decimales'], $idioma->fields['separador_decimales'], $idioma->fields['separador_miles']), $html2);
					if (( ( method_exists('Conf', 'GetConf') && Conf::GetConf($this->sesion, 'UsarImpuestoSeparado') ) || ( method_exists('Conf', 'UsarImpuestoSeparado') && Conf::UsarImpuestoSeparado() ))) {
						$html2 = $this->ReemplazarAnchor($theTag,'%monto_impuestos%', number_format($impuesto + $impuesto_gastos, $moneda_factura->fields['cifras_decimales'], $idioma->fields['separador_decimales'], $idioma->fields['separador_miles']), $html2);
					} else {
						$html2 = $this->ReemplazarAnchor($theTag,'%monto_impuestos%', '', $html2);
					}
					if (( ( method_exists('Conf', 'GetConf') && Conf::GetConf($this->sesion, 'UsarImpuestoSeparado') ) || ( method_exists('Conf', 'UsarImpuestoSeparado') && Conf::UsarImpuestoSeparado() ))) {
						$html2 = $this->ReemplazarAnchor($theTag,'%monto_total_brutto%', number_format($total + $monto_gastos, $moneda_factura->fields['cifras_decimales'], $idioma->fields['separador_decimales'], $idioma->fields['separador_miles']), $html2);
					} else {
						$html2 = $this->ReemplazarAnchor($theTag,'%monto_total_brutto%', '', $html2);
					}
				}
				...
				if ($mostrar_honorarios) {
					$html2 = $this->ReemplazarAnchor($theTag,'%simbolo_honorarios_sin_impuesto%', '%simbolo_honorarios%', $html2);
					$html2 = $this->ReemplazarAnchor($theTag,'%simbolo_honorarios_con_impuesto%', '%simbolo_honorarios%', $html2);


					/* debe mostrar ceros en los espacios que sea 0 */
					$html2 = $this->ReemplazarAnchor($theTag,'%monto_honorarios_sin_impuesto%', number_format($honorarios_sin_impuesto, $moneda_factura->fields['cifras_decimales'], $idioma->fields['separador_decimales'], $idioma->fields['separador_miles']), $html2);
					$html2 = $this->ReemplazarAnchor($theTag,'%monto_honorarios_con_impuesto%', number_format($honorarios_con_impuesto, $moneda_factura->fields['cifras_decimales'], $idioma->fields['separador_decimales'], $idioma->fields['separador_miles']), $html2);
				} else {
					 ...
					 */
					$html2 = $this->ReemplazarAnchor($theTag,'%simbolo_honorarios_sin_impuesto%', '', $html2);
					$html2 = $this->ReemplazarAnchor($theTag,'%simbolo_honorarios_con_impuesto%', '', $html2);

					$html2 = $this->ReemplazarAnchor($theTag,'%monto_honorarios_sin_impuesto%', '', $html2);
					$html2 = $this->ReemplazarAnchor($theTag,'%monto_honorarios_con_impuesto%', '', $html2);
				}

				if ($gastos_con_impuesto > 0) {

					$html2 = $this->ReemplazarAnchor($theTag,'%subtotal_gasto_con_impuesto%', number_format($gastos_con_impuesto, $moneda_factura->fields['cifras_decimales'], $idioma->fields['separador_decimales'], $idioma->fields['separador_miles']), $html2);
				} else {

					$html2 = $this->ReemplazarAnchor($theTag,'%simbolo_subtotal_gastos_con_impuesto%', '', $html2);
					$html2 = $this->ReemplazarAnchor($theTag,'%subtotal_gasto_con_impuesto%', '', $html2);
				}

				if ($gastos_sin_impuesto > 0) {

					$html2 = $this->ReemplazarAnchor($theTag,'%subtotal_gasto_sin_impuesto%', number_format($gastos_sin_impuesto, $moneda_factura->fields['cifras_decimales'], $idioma->fields['separador_decimales'], $idioma->fields['separador_miles']), $html2);
				} else {

					$html2 = $this->ReemplazarAnchor($theTag,'%simbolo_subtotal_gastos_sin_impuesto%', '', $html2);
					$html2 = $this->ReemplazarAnchor($theTag,'%subtotal_gasto_sin_impuesto%', '', $html2);
				}

				$html2 = $this->ReemplazarAnchor($theTag,'%glosa_banco%', $glosa_banco, $html2);
				$html2 = $this->ReemplazarAnchor($theTag,'%total_exentos%', number_format($subtotal_exentos, $moneda_factura->fields['cifras_decimales'], $idioma->fields['separador_decimales'], $idioma->fields['separador_miles']), $html2);
				$html2 = $this->ReemplazarAnchor($theTag,'%total_diez%', number_format($subtotal_diez, $moneda_factura->fields['cifras_decimales'], $idioma->fields['separador_decimales'], $idioma->fields['separador_miles']), $html2);
				$html2 = $this->ReemplazarAnchor($theTag,'%total_paraguay%', number_format($subtotal_completo, $moneda_factura->fields['cifras_decimales'], $idioma->fields['separador_decimales'], $idioma->fields['separador_miles']), $html2);
				$html2 = $this->ReemplazarAnchor($theTag,'%ceros%', number_format(0, $moneda_factura->fields['cifras_decimales'], $idioma->fields['separador_decimales'], $idioma->fields['separador_miles']), $html2);

				$html2 = $this->ReemplazarAnchor($theTag,'%iva_diez%', number_format($monto_impuesto_honorarios + $monto_impuesto_gastos, $moneda_factura->fields['cifras_decimales'], $idioma->fields['separador_decimales'], $idioma->fields['separador_miles']), $html2);

				$html2 = $this->ReemplazarAnchor($theTag,'%total_iva%', number_format($monto_impuesto_honorarios + $monto_impuesto_gastos, $moneda_factura->fields['cifras_decimales'], $idioma->fields['separador_decimales'], $idioma->fields['separador_miles']), $html2);
				...
				if (UtilesApp::GetConf($this->sesion, 'NuevoModuloFactura')) {
					...
					if ($mostrar_honorarios) {
						$html2 = $this->ReemplazarAnchor($theTag,'%honorarios%', number_format($monto_subtotal, $moneda_factura->fields['cifras_decimales'], $idioma->fields['separador_decimales'], $idioma->fields['separador_miles']), $html2);
					} else {
						$html2 = $this->ReemplazarAnchor($theTag,'%honorarios%', '', $html2);
					}
					...
					if ($mostrar_gastos_con_impuesto) {
						$html2 = $this->ReemplazarAnchor($theTag,'%simbolo_subtotal_gastos_con_impuesto%', $simbolo, $html2);
						if (UtilesApp::GetConf($this->sesion, 'UsarGlosaFacturaMayusculas')) {
							$html2 = $this->ReemplazarAnchor($theTag,'%descripcion_subtotal_gastos_con_impuesto%', strtoupper($descripcion_subtotal_gastos_separado), $html2);
						} else {
							$html2 = $this->ReemplazarAnchor($theTag,'%descripcion_subtotal_gastos_con_impuesto%', $descripcion_subtotal_gastos_separado, $html2);
						}
						$html2 = $this->ReemplazarAnchor($theTag,'%subtotal_gastos_con_impuesto%', number_format($subtotal_gastos_con_impuesto, $moneda_factura->fields['cifras_decimales'], $idioma->fields['separador_decimales'], $idioma->fields['separador_miles']), $html2);
					} else {
						$html2 = $this->ReemplazarAnchor($theTag,'%simbolo_subtotal_gastos_con_impuesto%', ' ', $html2);
						$html2 = $this->ReemplazarAnchor($theTag,'%descripcion_subtotal_gastos_con_impuesto%', ' ', $html2);
						$html2 = $this->ReemplazarAnchor($theTag,'%subtotal_gastos_con_impuesto%', ' ', $html2);
					}

					if (UtilesApp::GetConf($this->sesion, 'UsarGastosConSinImpuesto')) {
						if ($mostrar_gastos_sin_impuesto) {
							$html2 = $this->ReemplazarAnchor($theTag,'%simbolo_subtotal_gastos_sin_impuesto%', $simbolo, $html2);
							if (UtilesApp::GetConf($this->sesion, 'UsarGlosaFacturaMayusculas')) {
								$html2 = $this->ReemplazarAnchor($theTag,'%descripcion_subtotal_gastos_sin_impuesto%', strtoupper($descripcion_subtotal_gastos_sin_impuesto_separado), $html2);
							} else {
								$html2 = $this->ReemplazarAnchor($theTag,'%descripcion_subtotal_gastos_sin_impuesto%', $descripcion_subtotal_gastos_sin_impuesto_separado, $html2);
							}

							$html2 = $this->ReemplazarAnchor($theTag,'%subtotal_gastos_sin_impuesto%', number_format($subtotal_gastos_sin_impuesto, $moneda_factura->fields['cifras_decimales'], $idioma->fields['separador_decimales'], $idioma->fields['separador_miles']), $html2);
						} else {
							$html2 = $this->ReemplazarAnchor($theTag,'%simbolo_subtotal_gastos_sin_impuesto%', '', $html2);
							$html2 = $this->ReemplazarAnchor($theTag,'%descripcion_subtotal_gastos_sin_impuesto%', '', $html2);
							$html2 = $this->ReemplazarAnchor($theTag,'%subtotal_gastos_sin_impuesto%', '', $html2);
						}
					} else {
						$html2 = $this->ReemplazarAnchor($theTag,'%simbolo_subtotal_gastos_sin_impuesto%', '', $html2);
						$html2 = $this->ReemplazarAnchor($theTag,'%descripcion_subtotal_gastos_sin_impuesto%', '', $html2);
					...
					$html2 = $this->ReemplazarAnchor($theTag,'%suma_montos_sin_iva%', $suma_monto_sin_iva, $html2);
					$html2 = $this->ReemplazarAnchor($theTag,'%suma_montos_solo_iva%', $impuesto, $html2);
					$html2 = $this->ReemplazarAnchor($theTag,'%suma_monto_con_iva%', $suma_monto_con_iva, $html2);

					$monto_subtotal_honorario_y_gastos = $monto_subtotal + $subtotal_gastos + $subtotal_gastos_sin_impuesto;
					$html2 = $this->ReemplazarAnchor($theTag,'%monto_subtotal_honorario_y_gastos%', number_format($monto_subtotal_honorario_y_gastos, $moneda_factura->fields['cifras_decimales'], $idioma->fields['separador_decimales'], $idioma->fields['separador_miles']), $html2);
				} else {
					...
					$html2 = $this->ReemplazarAnchor($theTag,'%suma_montos_sin_iva%', '', $html2);
					$html2 = $this->ReemplazarAnchor($theTag,'%suma_montos_solo_iva%', '', $html2);
					$html2 = $this->ReemplazarAnchor($theTag,'%suma_monto_con_iva%', '', $html2);
					$html2 = $this->ReemplazarAnchor($theTag,'%monto_subtotal_honorario_y_gastos%', '', $html2);
				}
				...
				if ($mostrar_honorarios) {
					$html2 = $this->ReemplazarAnchor($theTag,'%simbolo_honorarios%', $simbolo, $html2);
				} else {
					$html2 = $this->ReemplazarAnchor($theTag,'%simbolo_honorarios%', '', $html2);
				}
				$html2 = $this->ReemplazarAnchor($theTag,'%simbolo%', $simbolo, $html2);

				$html2 = $this->ReemplazarAnchor($theTag,'%subtotal%', number_format($monto_subtotal, $moneda_factura->fields['cifras_decimales'], $idioma->fields['separador_decimales'], $idioma->fields['separador_miles']), $html2);
				$html2 = $this->ReemplazarAnchor($theTag,'%monto_impuesto_sin_gastos%', number_format($impuesto, $moneda_factura->fields['cifras_decimales'], $idioma->fields['separador_decimales'], $idioma->fields['separador_miles']), $html2);
				$html2 = $this->ReemplazarAnchor($theTag,'%monto_total_bruto_sin_gastos%', number_format($total, $moneda_factura->fields['cifras_decimales'], $idioma->fields['separador_decimales'], $idioma->fields['separador_miles']), $html2);
				$html2 = $this->ReemplazarAnchor($theTag,'%monto_total_palabra%', $monto_total_palabra, $html2);
			...
			case 'BOTTOM_COPIA':
				...
				$html2 = $this->ReemplazarAnchor($theTag,'%linea1%', $PdfLinea1, $html2);
				$html2 = $this->ReemplazarAnchor($theTag,'%numeracion%', $numeracion, $html2);
				$meses_org = array('Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec');
				$meses_largo = array('ENERO', 'FEBRERO', 'MARZO', 'ABRIL', 'MAYO', 'JUNIO', 'JULIO', 'AGOSTO', 'SEPTIEMBRE', 'OCTUBRE', 'NOVIEMBRE', 'DICIEMBRE');
				$html2 = $this->ReemplazarAnchor($theTag,'%fecha_numeracion%', str_replace($meses_org, $meses_largo, date('M j Y', strtotime($numeracion_fecha))), $html2);
				$html2 = $this->ReemplazarAnchor($theTag,'%numeracion_desde%', $numeracion_desde, $html2);
				$html2 = $this->ReemplazarAnchor($theTag,'%numeracion_hasta%', $numeracion_hasta, $html2);
				$html2 = $this->ReemplazarAnchor($theTag,'%titulo%', $titulo_empresa, $html2);
				$html2 = $this->ReemplazarAnchor($theTag,'%subtitulo%', $subtitulo_empresa, $html2);
				break;
		}
...
//		}
	...
	public function QueryReporte($orden, $where, $numero, $fecha1, $fecha2
		...
		if ($where == '') {
			...
			if (UtilesApp::GetConf($this->sesion, 'CodigoSecundario') && $codigo_asunto_secundario) {
		...
		$query = "SELECT SQL_CALC_FOUND_ROWS
		   ...
		   LEFT JOIN prm_estudio ON factura.id_cia=prm_estudio.id_cia
		   WHERE $where
	...
	public function ReemplazarAnchor($seccion,$anchor,$valor,$html) {
		 if(strpos($anchor,'<br>')===false) $this->arrayanchors[$seccion][str_replace(array('%','%|'),array(''),$anchor)]=(str_replace(array('<BR>','<BR/>','<br>',"\n"),'' , $valor));
		 return str_replace($anchor,$valor,$html);
	}
	
	/**
	 * Descarga el Documento (Word u OpenOffice) con el contenido de la factura
	 */
	public function DownloadWord() {
		$this->Load($this->fields[$this->campo_id]);
		...
		$mime_type = $fileinfo->buffer($template->fields['documento']);
		 if(strpos($mime_type,'vnd.oasis.opendocument.text')===true) { //es open office
		 	$template->Download("Documento_Legal_".$this->fields[$this->campo_id].".odt", $datos,'odt');
		 } else {
		 	$template->Download("Documento_Legal_".$this->fields[$this->campo_id].".docx", $datos,'docx');
		 }
	...
	public function FillTemplate() {
		...
		$query = "";
				...
				$parser = new TemplateParser($templateData);
				$html = $this->GenerarDocumento($parser, 'CARTA_FACTURA', $lang);
//echo '<pre>';print_r($this->arrayanchors);echo '</pre>';die();
		return $this->arrayanchors;
	}
