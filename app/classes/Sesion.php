...
class Sesion extends \Sesion {
	...
	public function LoadLang() {
		...
		if (!$existememcache || !$langs = @unserialize($memcache->get('cachedlangs' . LLAVE))) {
			...
			try {
				...
				if ($existememcache) {
					$memcache->set('cachedlangs' . LLAVE, serialize($archivos), false, 60);
				}
		...
		foreach ($langs as $lang) {
			if (is_readable(Conf::ServerDir() . '/lang/' . $lang)) {
				include_once Conf::ServerDir() . '/lang/' . $lang;
			}
		}
	}
	...
	public function LoadPlugin() {
		...
		if (!$existememcache || !$archivos = @unserialize($memcache->get('cachedplugins' . LLAVE))) {
			...
			if ($existememcache) {
				$memcache->set('cachedplugins' . LLAVE, serialize($archivos), false, 60);
			}
		}
		if (count($plugins) == 0) {
			return;
		...
		foreach ($plugins as $plugin) {
			if (is_readable(Conf::ServerDir() . '/plugins/' . $plugin)) {
