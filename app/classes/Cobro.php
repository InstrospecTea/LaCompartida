...
class Cobro extends Objeto {
	...
	function CalcularEstadoAnterior() {
		/*
		 * este estado anterior es s�lo cuando no tiene pagos. 
		...
		$estado_anterior_temp = 'EMITIDO'; /* estado m�s atr�s que puede llegar al borrar pago */
	...
	function CambiarEstadoSegunFacturas() {
		/*
		  ...
		  ENVIADO AL CLIENTE	si no hay facturas y pas� por el estado enviado al cliente, dejarlo asi
	...
	function GuardarCobro($emitir = false, $mantener_porcentaje_impuesto = false) {
		if ($this->fields['estado'] != 'CREADO' AND $this->fields['estado'] != 'EN REVISION' AND $this->fields['estado'] != '')
			return "No se puede guardar " . __('el cobro') . " ya que ya se encuentra emitido. Usted debe volver " . __('el cobro') . " a estado creado o en revisi�n para poder actualizarlo";
		...
		$cobro_total_honorario_cobrable = 0; // Valor real que se va a cobrar (seg�n forma de cobro), sin descuentos
		$cobro_total_gastos = 0; // Valor total de los gastos (egresos) del cobro
		$cobro_total_honorarios_hh_incluidos_retainer = 0; //Honorarios que dejan de cobrarse a HH pq est�n incluidos en retainer
		...
		for ($z = 0; $z < $lista_tramites->num; $z++) {
			...
			if (!$tramite->Write())
				return 'Error, tr�mite #' . $tramite->fields['id_tramite'] . ' no se pudo guardar';
		}
		 ...
		 * Eso vamos a conseguir con la definici�n de las variables $select_retainer_usuarios y $orden_retainer_usuarios
		 */
		...
		if ($this->fields['forma_cobro'] == 'ESCALONADA') {
			...
			for ($z = 0; $z < $lista_trabajos->num; $z++) {
				...
				// Se obtiene la tarifa del profesional que hizo el trabajo (s�lo si no se tiene todav�a).
				...
				if (UtilesApp::GetConf($this->sesion, 'GuardarTarifaAlIngresoDeHora')) {
					...
					// Seg�n Tarifa est�ndar del sistema
					...
					// Seg�n tarifa est�ndar de sistema y si no existe tarifa en moneda indicada buscar mejor tarifa en otra moneda
				...
				// Se calcula el valor del trabajo, seg�n el tiempo trabajado y la tarifa
				...
				// El valor a cobrar del trabajo depender� de la forma de cobro
				switch ($this->fields['forma_cobro']) {
					case 'FLAT FEE':
						$valor_a_cobrar = 0;
						break;
					case 'PROPORCIONAL':
						// Se calculan despu�s, pero necesitan los valores de los totales que se suman en este ciclo.
						continue;
						break;
					case 'RETAINER':
				...
				// Se guarda la informaci�n del cobro para este trabajo. se incluye minutos retainer en el trabajo
		...
		if ($this->fields['forma_cobro'] == 'PROPORCIONAL') {
			for ($z = 0; $z < $lista_trabajos->num; ++$z) {
				...
				if ($trabajo->fields['cobrable'] == '0') {
					...
					// Valor a cobrar proporcional a la fracci�n de horas del total asignadas a este trabajo.
				...
				// Se suma el monto a cobrar
				$cobro_total_honorario_cobrable += $valor_a_cobrar;
				// Se guarda la informaci�n del cobro para este trabajo. se incluye minutos y segundos retainer en el trabajo
				$horas_retainer = floor($retainer_trabajo_minutos / 60);
				$minutos_retainer = sprintf("%02d", $retainer_trabajo_minutos % 60);
		...
		/* Si las Provisiones superan al Gasto, se debe generar un Gasto por esa cantidad, de modo que total_gastos quede en 0, y se debe crear una provisi�n igual a ese gasto, para incluir en cobro futuro */
		...
		// Se guarda la informaci�n del cobro
	...
	/* Funci�n One-shot que vuelve a crear un documento como cuando fue emitido */
	...
	/*
	  Asocia los trabajos al cobro que se est� creando
	...
	function PrepararCobro($fecha_ini = '', $fecha_fin, $id_contrato, $emitir_obligatoriamente = false, $id_proceso, $monto='', $id_cobro_pendiente='', $con_gastos=false, $solo_gastos=false, $incluye_gastos=true, $incluye_honorarios=true) {
		...
		if ($contrato->Load($id_contrato)) {
			...
			if ($genera) {
				...
				// Pasar configuraci�n de escalonadas ... 
				...
				/**
				 * Configuraci�n moneda del cobro
				 */
	...
	function GenerarDocumentoCarta($parser_carta, $theTag='', $lang, $moneda_cliente_cambio, $moneda_cli, & $idioma, $moneda, $moneda_base, $trabajo, & $profesionales, $gasto, & $totales, $tipo_cambio_moneda_total, $cliente, $id_carta) {
		...
		switch ($theTag) {
			...
			case 'FECHA':
				/* DICTIONARIO
				 * %fecha_especial%  			---  CIUDAD, DIA de MES de A�O
				 * %fecha%					 			---  MES DIA, A�O
				 * %fecha_con_de% 	 			---  MES D�A de A�O
				 * %fecha_ingl�s% 				---  MES D�A, A�O
				 * %fecha_espanol%                              ---  D�A de MES, A�O
			...
			case 'ENVIO_DIRECCION':
				 ...
				 * %nombre_cliente%        ---  Glosa Cliente - directamente de tabla cliente
				 ...
				 * %CodigoAsunto%         ---  Codigo del asunto si es que hay solo uno si m�s reemplaza con string vacio
				 * %pais%                 ---  Chile
				 * %num_letter%           ---  ID Cobro
				 * %num_letter_documento% ---  Numero de documentos asociados
				 * %num_letter_baz%       ---  -""-
				 * %sr%                   ---  Imprime el titulo del contacto definido en el contrato, por defecto "Se�or"
				 * %asunto_mb%            ---  Ref.:
				...
				$html2 = str_replace('%valor_direccion_uc%', ucwords(strtolower(nl2br($contrato->fields['direccion_contacto']))), $html2);
			...
			case 'DETALLE':
				/* DICTIONARIO
				 * %saludo_mb%               --- Dear %sr% %ApellidoContacto%: / De mi consideraci�n:
				...
				if ($total_gastos != 0) {
					if ($this->fields['monto_subtotal'] > 0) {
						if (( ( method_exists('Conf', 'GetConf') && Conf::GetConf($this->sesion, 'ValorSinEspacio') ) || ( method_exists('Conf', 'ValorSinEspacio') && Conf::ValorSinEspacio() )))
							$detalle_cuenta_gastos = __('; m�s') . ' (ii) ' . $moneda_total->fields['simbolo'] . number_format($total_gastos, $moneda_total->fields['cifras_decimales'], $idioma->fields['separador_decimales'], $idioma->fields['separador_miles']) . ' ' . __('por concepto de gastos incurridos por nuestro Estudio en dicho per�odo');
						else
							$detalle_cuenta_gastos = __('; m�s') . ' (ii) ' . $moneda_total->fields['simbolo'] . ' ' . number_format($total_gastos, $moneda_total->fields['cifras_decimales'], $idioma->fields['separador_decimales'], $idioma->fields['separador_miles']) . ' ' . __('por concepto de gastos incurridos por nuestro Estudio en dicho per�odo');
					}
					else
					...
					if (( ( method_exists('Conf', 'GetConf') && Conf::GetConf($this->sesion, 'ValorSinEspacio') ) || ( method_exists('Conf', 'ValorSinEspacio') && Conf::ValorSinEspacio() )))
						$boleta_gastos = __('; m�s') . ' (ii) ' . $moneda_total->fields['simbolo'] . number_format($total_gastos, $moneda_total->fields['cifras_decimales'], $idioma->fields['separador_decimales'], $idioma->fields['separador_miles']) . __('por gastos a reembolsar') . __(', seg�n Boleta de Recuperaci�n de Gastos adjunta');
					else
						$boleta_gastos = __('; m�s') . ' (ii) ' . $moneda_total->fields['simbolo'] . ' ' . number_format($total_gastos, $moneda_total->fields['cifras_decimales'], $idioma->fields['separador_decimales'], $idioma->fields['separador_miles']) . ' ' . __('por gastos a reembolsar') . __(', seg�n Boleta de Recuperaci�n de Gastos adjunta');
					if (( ( method_exists('Conf', 'GetConf') && Conf::GetConf($this->sesion, 'ValorSinEspacio') ) || ( method_exists('Conf', 'ValorSinEspacio') && Conf::ValorSinEspacio() )))
						$detalle_cuenta_gastos2 = __('; m�s') . ' (ii) CH' . $moneda_total->fields['simbolo'] . number_format($total_gastos, $moneda_total->fields['cifras_decimales'], $idioma->fields['separador_decimales'], $idioma->fields['separador_miles']) . ' ' . __('por concepto de gastos incurridos por nuestro Estudio');
					else
						$detalle_cuenta_gastos2 = __('; m�s') . ' (ii) CH' . $moneda_total->fields['simbolo'] . ' ' . number_format($total_gastos, $moneda_total->fields['cifras_decimales'], $idioma->fields['separador_decimales'], $idioma->fields['separador_miles']) . ' ' . __('por concepto de gastos incurridos por nuestro Estudio');
				}
	...
	function GenerarDocumento($parser, $theTag='INFORME', $parser_carta, $moneda_cliente_cambio, $moneda_cli, $lang, $html2, & $idioma, $cliente, $moneda, $moneda_base, $trabajo, & $profesionales, $gasto, & $totales, $tipo_cambio_moneda_total, $asunto) {
		...
		switch ($theTag) {
			...
			case 'CLIENTE':
				...
				$html = str_replace('%valor_direccion_uc%', ucwords(strtolower($contrato->fields['factura_direccion'])), $html);
				...
				$html = str_replace('%atencion%', empty($contrato->fields['contacto']) ? '' : __('Atenci�n'), $html);
				if (method_exists('Conf', 'GetConf')) {
					if (Conf::GetConf($this->sesion, 'TituloContacto'))
				...
				$html = str_replace('%telefono%', empty($contrato->fields['fono_contacto']) ? '' : __('Tel�fono'), $html);
			...
			case 'TRAMITES_TOTAL':
				...
				$html = str_replace('%glosa_tramites%', __('Total Tr�mites'), $html);
	...
	function GenerarDocumento2($parser, $theTag='INFORME', $parser_carta, $moneda_cliente_cambio, $moneda_cli, $lang, $html2, & $idioma, $cliente, $moneda, $moneda_base, $trabajo, & $profesionales, $gasto, & $totales, $tipo_cambio_moneda_total, $asunto) {
		...
		switch ($theTag) {
			...
			case 'CLIENTE':
				...
				$html = str_replace('%a_min%', empty($contrato->fields['contacto']) ? '' : __('a'), $html);
				$html = str_replace('%cliente%', __('Cliente'), $html);
				$html = str_replace('%glosa_cliente%', $contrato->fields['factura_razon_social'], $html); #glosa cliente de factura
				$html = str_replace('%nombre_cliente%', $glosa_cliente, $html); #nombre real del cliente
			...
			case 'DETALLE_COBRO':
				...
				#valor en moneda previa selecci�n para impresi�n
				/*
				 * Implementaci�n funci�n procesa cobro id_moneda
			...
			case 'TRABAJOS_ENCABEZADO':
				...
				$html = str_replace('%categoria%', __('Categor�a'), $html);
			...
			case 'GASTOS_ENCABEZADO':
				...
				$html = str_replace('%monto_moneda_total%', __('Monto') . ' (' . $moneda_total->fields['simbolo'] . ')', $html);
				/*
				 * Implementaci�n Gastos con IVA y sin IVA
				 * acordados en la reuni�n del 4/02/2011
				 * JMBT
				 *
