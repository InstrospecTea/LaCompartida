...
if (!class_exists('Cobro')) {
	...
	class Cobro extends Objeto {
		...
		function Write($writeLog = true) {
			...
			$query = "UPDATE trabajo SET estadocobro = '{$this->fields['estado']}' WHERE id_cobro = '{$this->fields['id_cobro']}'";
		...
		function GlosaSeEstaCobrando() {
			...
			if (Conf::GetConf($this->sesion, 'SeEstaCobrandoEspecial')) {
				...
				$monedas = Moneda::GetMonedas($this->sesion, '', true);
		...
		function CambiarEstadoSegunFacturas() {
			...
			if ($num_facturas == 0 && Conf::GetConf($this->sesion, 'NuevoModuloFactura')) {
				...
				if (Conf::GetConf($this->sesion, 'NuevoModuloFactura')) {
					 ...
					 *
					 */
					$criteria->add_select('(SUM(IFNULL(CCFMN.monto_pago, 0)) - SUM(IFNULL(CCFMNP.monto_pago, 0)))', 'monto_pago_parcial')
										->add_from('cobro C')
		...
		function GuardarCobro($emitir = false, $mantener_porcentaje_impuesto = false) {
			...
			$_calcula_monto_tramites = $this->CalculaMontoTramites();
			...
			$this->Edit('monto_subtotal', number_format($_calcula_monto_tramites + $cobro_total_honorario_cobrable, 6/* $cobro_moneda->moneda[$this->fields['id_moneda']]['cifras_decimales'] */, ".", ""));
			...
			$this->Edit('monto_tramites', number_format($_calcula_monto_tramites, 6/* $cobro_moneda->moneda[$this->fields['id_moneda']]['cifras_decimales'] */, ".", ""));

			if ($cobro_total_honorario_cobrable != $cobro_total_honorario_cobrable_original && $cobro_total_honorario_cobrable != 0) {
				...
				if ($emitir) {
					...
					if ($documento->Write()) {
						...
						$query_documento_moneda = "REPLACE INTO documento_moneda (id_documento, id_moneda, tipo_cambio)
														SELECT :iddocumento, cobro_moneda.id_moneda, cobro_moneda.tipo_cambio
															FROM cobro
		...
		function TienePagosAdelantos() {
			$query = "SELECT COUNT(*) AS nro_pagos
		...
		function DetalleProfesional() {
			...
			$mostrarhorascero = Conf::GetConf($this->sesion, 'MostrarHorasCero');
