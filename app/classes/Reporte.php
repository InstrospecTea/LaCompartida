...
class Reporte {
	...
	public function Query() {
		$stringquery = "";
		$resp = mysql_unbuffered_query($this->getQuery(), $this->sesion->dbh) or Utiles::errorSQL($this->sQuery(), __FILE__, __LINE__, $this->sesion->dbh);
	...
	private function setSELECT($Criteria) {
		...
		if (in_array('prm_area_proyecto.glosa', $this->agrupador)) {
			$Criteria->add_select('prm_area_proyecto.glosa');
		}
		if (in_array('area_usuario', $this->agrupador)) {
			$Criteria->add_select('IFNULL(prm_area_usuario.glosa,\'-\')', 'area_usuario');
		}
		if (in_array('categoria_usuario', $this->agrupador)) {
			$Criteria->add_select('IFNULL(prm_categoria_usuario.glosa_categoria,\'-\')', 'categoria_usuario');
		}
		if (in_array('id_usuario_responsable', $this->agrupador)) {
