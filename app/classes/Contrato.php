...
class Contrato extends Objeto {
	...
	/**
	 * Verifica si el contrato puede ser eliminado
	 * @return boolean
	 */
	function CheckDelete() {
		...
		$id_contrato = $this->fields[$this->campo_id];
	...
	function MontoHonorariosEscalonados($lista_trabajos) {
		...
		for ($z = 0; $z < $lista_trabajos->num; $z++) {
			...
			if ($trabajo->fields['cobrable']) {
				// Revisa duración de la hora y suma duracion que sobro del trabajo anterior,
				// si es que se cambió de escalonada
	...
	function Prepare() {
		...
		if (Conf::GetConf($this->sesion, 'CopiarEncargadoAlAsunto')) {
	...
	function Fill($parametros, $edicion = false) {
		...
		if (Conf::GetConf($this->sesion, 'ExportacionLedes')) {
			...
			$this->Edit('formato_ledes', !empty($this->extra_fields['formato_ledes']) ? $this->extra_fields['formato_ledes'] : null);
		}
	...
	function Write($enviar_mail_asunto_nuevo = true) {
		...
		if ($this->Loaded()) {
			...
			// Solo en caso de que se haya modificado algún campo
			if ($do_update) {
				$resp = mysql_query($query, $this->sesion->dbh) or Utiles::errorSQL($query, __FILE__, __LINE__, $this->sesion->dbh);
				...
				// Retorna true ya que si no quiere hacer update la función corrió bien
				return true;
			}
		} else {
			...
			if ($enviar_mail_asunto_nuevo) {
				...
				if (Conf::GetConf($this->sesion, 'CorreosModificacionAdminDatos')) {
