...
QUERY;
		...
		case 7.82:
			$queries = array();
			$queries[] = "INSERT IGNORE INTO `menu` (`codigo`, `glosa`, `url`, `tipo`, `orden`, `codigo_padre`, `bitmodfactura`) VALUES ('AUDIT', 'Auditor�a', '/app/interfaces/reporte_historial_movimientos.php', '0', '99999', 'ADMIN_SIS', '0');";
		...
		case 7.83:
			...
			$queries[] = "INSERT IGNORE INTO `prm_excel_cobro` (`id_prm_excel_cobro`, `nombre_interno`, `grupo`, `glosa_es`, `glosa_en`, `tamano`) VALUES (NULL, 'concepto_glosa', 'Encabezado', 'Servicios profesionales prestados a la compa��a durante el mes de %s', 'Professional services provided to the company during %s', 0)";
		...
		case 7.89:
			$queries = array();
			$queries[] = "INSERT IGNORE INTO `configuracion` (`glosa_opcion`, `valor_opcion`, `comentario`, `valores_posibles`, `id_configuracion_categoria`, `orden`) VALUES ('OrdenarFacturasPorDefecto', 'numero desc', 'Campos soportados para ordenamiento:<br/> Raz�n Social => cliente<br/> Fecha Documento => fecha<br/> Datos Documentos => numero<br/> Socio a cargo => encargado_comercial<br/> N�Liquidaci�n => id_cobro<br/> Estado => estado', 'string', '6', '-1');";
			break;

		case 7.90:
			$queries = array();
			$queries[] = "INSERT IGNORE INTO `configuracion` (`glosa_opcion`, `valor_opcion`, `comentario`, `valores_posibles`, `id_configuracion_categoria`, `orden`) VALUES ('ValidacionesClienteExcepciones', '', 'Campos que no se validar�n', 'string', '6', '-1');";
			break;
		...
		case 7.94:
			...
			if (!ExisteCampo('desglose_area', 'asunto', $dbh)) {
				$queries[] = "ALTER TABLE `asunto` ADD `desglose_area` VARCHAR(255) NULL COMMENT 'este campo contiene desglose del �rea del asunto cuando corresponde' AFTER `id_area_proyecto` ;";
			}
			...
			if (!ExisteCampo('cotizado_con', 'asunto', $dbh)) {
				$queries[] = "ALTER TABLE `asunto` ADD `cotizado_con` VARCHAR(255) NULL COMMENT 'contiene una descripci�n de otro asunto cotizado en conjunto' AFTER `contraparte` ;";
			}
		...
		case 7.95:
			$queries[] = "INSERT IGNORE INTO `configuracion` ( `glosa_opcion`, `valor_opcion`, `comentario`, `valores_posibles`, `id_configuracion_categoria`, `orden`)
				VALUES ('UsaFechaDesdeUltimoCobro', '0', 'Permite definir autom�ticamente la fecha desde de los cobros seg�n la fecha del �ltimo cobro emitido + 1', 'boolean', '6', -1)";
			break;
		...
		case 8.00:
			...
			if (!ExisteCampo('codigo_homologacion', 'cliente', $dbh)) {
				...
				// Actualiza todos los clientes con el c�digo del cliente por defecto
		...
		case 8.06:
			...
			$queries[] = "ALTER TABLE `contrato`
				ADD `emitir_liquidacion_al_generar` int(11) NULL DEFAULT 0 COMMENT 'Liquidaciones Programadas: Emitir la liquidaci�n al generar',
				ADD `enviar_liquidacion_al_generar` int(11) NULL DEFAULT 0 COMMENT 'Liquidaciones Programadas: Enviar la liquidaci�n generada al cliente'";
		...
		case 8.11:
			...
			if ($archivo['archivo_nombre'] == 'facturacion_electronica_nubox.php') {
				$queries[] = "INSERT INTO prm_codigo (grupo, codigo, glosa) VALUES 
								('PRM_FACTURA_DR', '33', 'Factura Electr�nica'), 
								('PRM_FACTURA_DR', '34', 'Factura Electr�nica Exenta'), 
								('PRM_FACTURA_DR', '39', 'Boleta Electr�nica'), 
								('PRM_FACTURA_DR', '41', 'Boleta Electr�nica Exenta'), 
								('PRM_FACTURA_DR', '56', 'Nota de D�bito Electr�nica'), 
								('PRM_FACTURA_DR', '61', 'Nota de Cr�dito Electr�nica'), 
				...
				$queries[] = "INSERT INTO prm_codigo (grupo, codigo, glosa) VALUES 
								...
								('PRM_FACTURA_DR', '814', 'Certificaci�n de Dep�sito Bolsa Prod. Chile'),
								('PRM_FACTURA_DR', '815', 'Vale de Prenda Bolsa Prod. Chile'),
								('PRM_FACTURA_DR', 'HAS', 'Hoja Aceptaci�n Servicio (HAS)'),
								('PRM_FACTURA_DR', 'HES', 'Hoja Estado Servicio (HES)'),
								('PRM_FACTURA_DR', 'HEM', 'Recepci�n de Material (HEM)'),
								('PRM_FACTURA_DR', 'MER', 'Mercader�a (MER)'),
