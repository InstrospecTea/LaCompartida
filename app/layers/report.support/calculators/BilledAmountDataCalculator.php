 ...
 class BilledAmountDataCalculator extends AbstractProportionalDataCalculator {
	...
	function getReportWorkQuery(Criteria $Criteria) {
		$Criteria
			...
			->add_select('SUM((tarifa_hh * TIME_TO_SEC(duracion_cobrada) / 3600) * ((((documento.monto_trabajos / (documento.monto_trabajos + documento.monto_tramites)) * documento.subtotal_sin_descuento) * cobro_moneda_documento.tipo_cambio) / (IF(cobro.monto_thh > 0, cobro.monto_thh, IF(cobro.monto_trabajos > 0, cobro.monto_trabajos, 1)) * cobro_moneda_cobro.tipo_cambio)) * (cobro_moneda_cobro.tipo_cambio/cobro_moneda.tipo_cambio))', 'valor_cobrado');
