...
var GOOGIE_DEFAULT_LANG = "es";

function GoogieSpell(options) {
    this.options = this.defaultOptions(options);
    this.getDefaultLang();

    this.lang_to_word = this.options.org_lang_to_word;
...
GoogieSpell.prototype.getDefaultLang  = function() {
    var cookie_value;
    ...
    GOOGIE_CUR_LANG = this.options.lang || GOOGIE_CUR_LANG;
}
...
GoogieSpell.prototype.defaultOptions  = function(opts) {
    return {
        ...
        show_change_lang_pic: opts.show_change_lang_pic || false,
        show_error_window: opts.show_error_window || false,
        change_lang_pic_placement: opts.change_lang_pic_placement || "left",
        report_state_change: opts.report_state_change || true,
        lang_chck_spell: opts.lang_chck_spell || "ORTOGRAFÍA",
        lang_revert: opts.lang_revert || "Cambiar a",
        lang_close: opts.lang_close || "Cerrar",
        lang_rsm_edt: opts.lang_rsm_edt || "Continuar editando",
        lang_no_error_found: opts.lang_no_error_found || "No hay errores de ortografía",
        lang_no_suggestions: opts.lang_no_suggestions || "No hay sugerencias",
        show_spell_img: opts.show_spell_img || false,
        decoration: opts.decoration || false,
        use_close_btn: opts.use_close_btn || false,
        edit_layer_dbl_click: opts.edit_layer_dbl_click || true,
        report_ta_not_found: opts.report_ta_not_found || true,
        spelling_state_observer: opts.observer || null
    }
}


GoogieSpell.prototype.decorateTextarea = function(id, parent_id) {
    if(typeof(id) == "string")
    ...
    if (parent_id){
        if(typeof(parent_id) == "string")
            this.parent_element = AJS.$(parent_id);
        else
            this.parent_element = parent_id;
    }

    var r_width, r_height;

    if(this.text_area != null) {
        if(!AJS.isDefined(this.spell_container) && this.options.decoration) {
            var table = AJS.TABLE();
            var tbody = AJS.TBODY();
            var tr = AJS.TR();
            ...
            AJS.insertAfter(table, this.parent_element || this.text_area);

            //Set width
            AJS.setHeight(table, spell_container, r_height);
            AJS.setWidth(table, spell_container, r_width);

            spell_container.style.textAlign = "left";
        }

        this.checkSpellingState();
    }
    else
        if(this.options.report_ta_not_found)
            alert("Text area no encontrado");
}
...
GoogieSpell.prototype.setDecoration = function(bool) {
    this.options.decoration = bool;
}

GoogieSpell.prototype.dontUseCloseButtons = function() {
    this.options.use_close_btn = false;
}
...
GoogieSpell.prototype.setStateChanged = function(current_state) {
    ...
    if(this.options.spelling_state_observer != null && this.options.report_state_change)
        this.options.spelling_state_observer(current_state, this);
}

GoogieSpell.prototype.setReportStateChange = function(bool) {
    this.options.report_state_change = bool;
}
...
GoogieSpell.prototype.getGoogleUrl = function() {
    return this.options.server_url + GOOGIE_CUR_LANG;
}
...
GoogieSpell.createJSONReq = function (text) {
    return {text: text};
}
...
GoogieSpell.prototype.spellCheck = function(opts) {
    var me = this;
    var ignore = opts && opts.ignore ? opts.ignore : null;
    var on_success = opts && opts.success ? opts.success : null;
    var on_error = opts && opts.error ? opts.error : null;
    ...
    //if (this.options.show_error_window) {
    ...
    var reqfailed = function(res_txt, req) {
        if(me.custom_ajax_error)
            ...
            alert("Ocurrió un error al revisar la ortografía. Por favor intente nuevamente.");
    ...
    d.sendReq(GoogieSpell.createJSONReq(req_text));
}
...
GoogieSpell.prototype.showErrorWindow = function(elm, id) {
    ...
    if(!changed) {
        ...
        if(suggestions.length == 0) {
            ...
            var dummy = AJS.SPAN();
            dummy.innerHTML = this.options.lang_no_suggestions;
        ...
        if(elm.is_changed && elm.innerHTML != elm.old_value) {
            ...
            rev_span.innerHTML = this.options.lang_revert + " " + old_value;
    ...
    if(AJS.isIe() && !this.error_window_iframe) {
        var iframe = AJS.IFRAME({'style': 'position: fixed; z-index: 0;'});
...
GoogieSpell.prototype.createEditLayer = function(width, height) {
    ...
    if(AJS.nodeName(this.text_area) != "input" || this.getValue(this.text_area) == "") {
        ...
        AJS.setHeight(this.edit_layer, (height));
    }
    else {
        this.edit_layer.style.overflow = "hidden";
    }

    if(this.options.edit_layer_dbl_click) {
        var me = this;
        var fn = function(e) {
...
GoogieSpell.prototype.isLangWindowShown = function() {
    return this.language_window != null && this.language_window.style.display == "inline";
}

GoogieSpell.prototype.hideLangWindow = function() {
    try {
        this.language_window.style.display = "none";
...
GoogieSpell.prototype.showLangWindow = function(elm, ofst_top, ofst_left) {
    ...
    this.highlightCurSel();
    this.language_window.style.display = "inline";
}

GoogieSpell.prototype.createChangeLangPic = function() {
    var img = AJS.IMG({'src': this.options.img_dir + 'change_lang.gif', 'alt': "Change language"});
...
GoogieSpell.prototype.createSpellDiv = function() {
    ...
    chk_spell.innerHTML = this.options.lang_chck_spell;
    var spell_img = null;
    if(this.options.show_spell_img)
        spell_img = AJS.IMG({'src': this.options.img_dir + "spellc.gif"});
...
GoogieSpell.prototype.flashNoSpellingErrorState = function(on_finish) {
    ...
    if(this.main_controller) {
        ...
        var dummy = AJS.IMG({'src': this.options.img_dir + "blank.gif", 'style': 'height: 16px; width: 1px;'});
        var rsm = AJS.SPAN();
        rsm.innerHTML = this.options.lang_no_error_found;
...
GoogieSpell.prototype.resumeEditingState = function() {
    ...
    if(this.main_controller) {
        ...
        var dummy = AJS.IMG({'src': this.options.img_dir + "blank.gif", 'style': 'height: 16px; width: 1px;'});
        var rsm = AJS.SPAN();
        rsm.innerHTML = this.options.lang_rsm_edt;
        AJS.RCN(this.spell_span, AJS.SPAN(dummy, rsm));
...
GoogieSpell.prototype.checkSpellingState = function(fire) {
    ...
    if(this.options.show_change_lang_pic)
    ...
    if(this.main_controller) {
        if(this.options.change_lang_pic_placement == "left")
...
GoogieSpell.prototype.createCloseButton = function(c_fn) {
    return this.createButton(this.options.lang_close, 'googie_list_close', AJS.$b(c_fn, this));
}
...
GoogieSpell.prototype.appendIndicator = function(elm) {
    var img = AJS.IMG({'src': this.options.img_dir + 'indicator.gif', 'style': 'margin-right: 5px;'});
    AJS.setWidth(img, 16);
