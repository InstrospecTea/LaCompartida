...
if ($AtacheSecundarioSoloAsunto) {
	...
	$regularizacion = $sesion->pdodbh->exec("UPDATE contrato INNER JOIN cliente USING(codigo_cliente) SET contrato.id_usuario_secundario = cliente.id_usuario_encargado
...
if ($xls) {
	...
	foreach ($arreglo_monedas as $id_m => $m) {
		...
		$formatos_moneda[$id_m] = & $wb->addFormat(array('Size' => 11, 'VAlign' => 'top', 'Align' => 'right', 'Border' => '1', 'Color' => 'black', 'NumFormat' => "[$" . $m['simbolo'] . "] #,###, 0$decimales"));
	}
	...
	foreach ($arreglo_monedas as $id_m => $m) {
		...
		$formatos_moneda_tc[$id_m] = & $wb->addFormat(array('Size' => 11, 'VAlign' => 'top', 'Align' => 'right', 'Border' => '1', 'Color' => 'black', 'NumFormat' => "[$" . $m['simbolo'] . "] #,###,0.00"));
	}
	...
	$formato_moneda_base_rojo = & $wb->addFormat(array('Size' => 11, 'VAlign' => 'top', 'Align' => 'right', 'Border' => 1, 'Color' => 'red', 'NumFormat' => '[$' . $moneda_base['simbolo'] . "] #,###,0$decimales"));
	...
	while ($cobro = mysql_fetch_array($respcobro)) {
		...
		if ($cobro['porcentaje_descuento'] > 0) {
			...
			// Evito pasar por aquí si es CAP, porque ya se hizo el cálculo del monto utilizado
			$valor_estimado = $valor_estimado - $valor_descuento;
