<?
	...
	if($xls)
	{
		...
		$query_usuarios_total = "SELECT trabajo.id_usuario, usuario.nombre, usuario.apellido1, usuario.apellido2  
												   		FROM trabajo 
		...
		while($cobro = mysql_fetch_array($resp))
		{
			...
			if(!$tabla_creada)
			{
				...
				$ws1->write($filas, $col_total_cobro_tarifa_estandar, __('Total según tarifa estandar'), $titulo_filas);
				$ws1->write($filas, $col_honorarios_estandar, __('Honorarios según tarifa estandar'), $titulo_filas);
			...
			if(  ( ( method_exists('Conf','GetConf') && Conf::GetConf($sesion,'UsarImpuestoSeparado') ) ||  ( method_exists('Conf','UsarImpuestoSeparado') && Conf::UsarImpuestoSeparado() )  ) ) 
				{
					if(  ( ( method_exists('Conf','GetConf') && Conf::GetConf($sesion,'UsarImpuestoPorGastos') ) || ( method_exists('Conf','UsarImpuestoPorGastos') && Conf::UsarImpuestoPorGastos() ) )  )
						$aproximacion_iva = number_format(($cobro['monto_subtotal']-$cobro['descuento'])*$cobro['porcentaje_impuesto']/100+($aproximacion_gastos)*$cobro['porcentaje_impuesto_gastos']/100,$cobro['cifras_decimales'],'.','');
					else
			 ...
			 */
			$cambio_moneda_en_formula = $cobro_moneda->moneda[$cobro['id_moneda']]['tipo_cambio']/$cobro['tipo_cambio'];
			if($cobro['modalidad_calculo']==1){
				$x_resultados = UtilesApp::ProcesaCobroIdMoneda($sesion, $cobro['id_cobro']);
				/*
				 *  IMPORTANTE, esta linea no descomentar hasta que DocumentoMoneda este funcionando en un 100%
				 *
				//$cambio_moneda_en_formula = $x_resultados['tipo_cambio_id_moneda']/$x_resultados['tipo_cambio_opc_moneda_total'];
				 * 
				 */
			}
			foreach( $abogados as $abogado => $data )
					...
					if($abogados_horas[$data['id']]['id'])
						{
							...
							$ws1->writeFormula($filas, $col_valor_moneda_usuario[$data['id']], "=".$col_formula_valor_usuario[$data['id']].($filas+1)."*".$cambio_moneda_en_formula, $i%2==0 ? $formatos_moneda_color[$moneda] : $formatos_moneda[$moneda]);
						}
					else
						{
			...
			/*
			 * Uso e implementacion de  FunciónProcesaCobroIdMoneda
			 */
			if($cobro['modalidad_calculo']==1){
				/*
				 *  Total Cobro Original				[id_moneda]
				 *  Total según tarifa estandar			[id_moneda]
				 *  Honorarios según tarifa estandar	[id_moneda]
				 * 
				 *  Total facturado	[moneda_seleccionada]
				 *  Honorarios		[moneda_seleccionada]
				 *  Gastos			[moneda_seleccionada]
				 *  IVA				[moneda_seleccionada]
				 */
				//$aproximacion_monto_tarifa_estandar	=$x_resultados['monto'][$cobro['id_moneda']];
				//$aproximacion_monto_tarifa_estandar_honorarios	=$x_resultados['monto_honorarios'][$cobro['id_moneda']];

				$monto_moneda = $x_resultados['monto_honorarios'][$moneda];
				if((( method_exists('Conf','GetConf') && Conf::GetConf($sesion,'UsarImpuestoPorGastos') ) || ( method_exists('Conf','UsarImpuestoPorGastos') && Conf::UsarImpuestoPorGastos()))){
					$monto_gastos = $x_resultados['subtotal_gastos'][$moneda];
					$aproximacion_monto = $x_resultados['monto_honorarios'][$cobro['id_moneda']] + $x_resultados['subtotal_gastos'][$cobro['id_moneda']] +  $x_resultados['monto_iva'][$cobro['id_moneda']];
				}
				else{
					$monto_gastos = $x_resultados['monto_gastos'][$moneda];
					$aproximacion_monto = $x_resultados['monto_honorarios'][$cobro['id_moneda']] + $x_resultados['monto_gastos'][$cobro['id_moneda']] +  $x_resultados['monto_iva'][$cobro['id_moneda']];
				}
				$monto_iva = $x_resultados['monto_iva'][$moneda];
			}
