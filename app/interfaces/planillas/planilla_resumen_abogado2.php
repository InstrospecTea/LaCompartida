<?
  ...
  if( ( ( method_exists('Conf','GetConf') && Conf::GetConf($sesion,'MostrarSoloMinutos') ) ||  ( method_exists('Conf','MostrarSoloMinutos') && Conf::MostrarSoloMinutos() )  ) )
	...
	$query="SELECT
					...
					trabajo.cobrable
					, GROUP_CONCAT(tarifa_hh) as arr_tarifa_hh
					, GROUP_CONCAT(TIME_TO_SEC(duracion_cobrada)/3600) as arr_duracion_cobrada
					, GROUP_CONCAT(cobro.id_moneda) as arr_id_moneda
					, cobro.id_moneda_base
					, GROUP_CONCAT(cobro.monto) as arr_monto
					, GROUP_CONCAT(cobro.monto_thh) as arr_monto_thh
					, GROUP_CONCAT(cobro.id_cobro) as arr_id_cobro
					, group_CONCAT(trabajo.id_trabajo) as arr_id_trabajo
					FROM trabajo
	...
	while($row = mysql_fetch_array($resp))
	{
		...
		$valor_cobrado=0;
		$monto_thh_valor_cobrado=0;
		$valor_tarifa_hh=0;
		$arr_tarifa_hh=array();
		$arr_duracion_cobrada=array();
		$arr_id_moneda=array();
		$arr_monto=array();
		$arr_monto_thh=array();
		$arr_tarifa_hh = explode(",",$row['arr_tarifa_hh']);
		$arr_id_moneda = explode(",",$row['arr_id_moneda']);
		$arr_monto = explode(",",$row['arr_monto']);
		$arr_monto_thh = explode(",",$row['arr_monto_thh']);
		$arr_id_cobro = explode(",",$row['arr_id_cobro']);
		$arr_id_trabajo = explode(",",$row['arr_id_trabajo']);
		$arr_duracion_cobrada = explode(",",$row['arr_duracion_cobrada']);
		if(count($arr_id_moneda) == 0) {
			$arr_id_moneda[0] = $row['arr_id_moneda'];
			$arr_tarifa_hh[0] = $row['arr_tarifa_hh'];
			$arr_monto[0] = $row['arr_monto'];
			$arr_monto_thh[0] = $row['arr_monto_thh'];
			$arr_id_cobro[0] = $row['arr_id_cobro'];
			$arr_id_trabajo[0] = $row['arr_id_trabajo'];
			$arr_duracion_cobrada[0] = $row['arr_duracion_cobrada'];
		}
		for($a=0;$a<count($arr_id_moneda);$a++){
			$cobro_moneda = new CobroMoneda($sesion);
			$cobro_moneda->Load( $arr_id_cobro[$a] );
			$valor_tarifa_hh = $arr_tarifa_hh[$a] * $arr_duracion_cobrada[$a];
			$valor_tarifa_hh = UtilesApp::CambiarMoneda($valor_tarifa_hh, $cobro_moneda->moneda[$arr_id_moneda[$a]]['tipo_cambio'], $cobro_moneda->moneda[$arr_id_moneda[$a]]['cifras_decimales'], $cobro_moneda->moneda[$row['id_moneda_base']]['tipo_cambio'], $cobro_moneda->moneda[$row['id_moneda_base']]['cifras_decimales']);
			if($arr_monto_thh[$a] > 0) {
			...
			$arr_result[$row['cliente_asunto']][$a]['arr_tarifa_hh']=$arr_tarifa_hh[$a];
			$arr_result[$row['cliente_asunto']][$a]['arr_id_moneda']=$arr_id_moneda[$a];
			$arr_result[$row['cliente_asunto']][$a]['arr_monto_thh']=$arr_monto_thh[$a];
			$arr_result[$row['cliente_asunto']][$a]['arr_monto']=$arr_monto[$a];
			$arr_result[$row['cliente_asunto']][$a]['monto_thh_valor_cobrado']=$monto_thh_valor_cobrado;
			$arr_result[$row['cliente_asunto']][$a]['valor_cobrado']=$valor_tarifa_hh * $monto_thh_valor_cobrado;
			$arr_result[$row['cliente_asunto']][$a]['valor_cobrado_acumulado']=$valor_cobrado;
			$arr_result[$row['cliente_asunto']][$a]['arr_id_trabajo']=$arr_id_trabajo[$a];
		}
		$arr_ordenado = asort($arr_result, 'valor_cobrado');
...
#echo "prof: $profesional fila: $fila columna: $periodo ";
			if($row['cobrado'] == 'SI' && $row['incobrable']=='NO')
			{
				...
				$ws1->writeNumber($fila,$columna_relativa+4,$valor_cobrado,$formato_cobro);
			}//aca
			else
