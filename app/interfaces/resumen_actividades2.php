...
function Generar(form, valor)
{
	...
	if(valor == 'pdf')
	{
		form.action = 'html_to_pdf.php?frequire=resumen_actividades2.php&popup=1';
	}
	else if(valor == 'op')
	{
		form.target = '_blank';
		form.action = 'resumen_actividades2.php?popup=1';
	}
	else
	{
		form.target = '_self';
		form.action = 'resumen_actividades2.php';
	}
...
<input type="hidden" name="horas_sql" id="horas_sql" value='<?=$horas_sql ? $horas_sql : 'horas_trabajadas' ?>'/>
		...
		?>
			<tr valign=top>
			 ...
			 <?php 
					...
					<option value='horas_trabajadas' <?=!$horas_sql ? 'selected':'' ?>><?=__('hr_trabajadas')?></option>
					<option value='horas_cobrables' <?=$horas_sql == 'horas_cobrables' ? 'selected':'' ?>><?=__('hr_cobrable')?></option>
					...
					<option value='horas_no_cobrables' <?=$horas_sql == 'horas_no_cobrables' ? 'selected':'' ?>><?=__('hr_no_cobrables')?></option>
					<option value='horas_castigadas' <?=$horas_sql == 'horas_castigadas' ? 'selected':'' ?>><?=__('hr_castigadas')?></option>
					<option value='horas_spot' <?=$horas_sql == 'horas_spot' ? 'selected':'' ?>><?=__('hr_spot')?></option>
					<option value='horas_convenio' <?=$horas_sql == 'horas_convenio' ? 'selected':'' ?>><?=__('hr_convenio')?></option>
				</select>
				</td>
			</tr>
			<tr>
...
<table border="1" cellpadding="3" class="planilla" id="tabla_planilla_2" width="100%" >
		...
		//magia para hacer un paso de manera simple, intenté hacerlo de una manera mejor, pero no funcionó después sigo intentando.
		function get_array_compara( $array_c, $array_k=array(), $pos_total_a ){
			$array_compara = array();
			
			switch ( $pos_total_a ){
				case 'b':
					$array_compara = $array_c[ $array_k[0] ][ $array_k[1] ];
					break;
				case 'c':
					$array_compara = $array_c[ $array_k[0] ][ $array_k[1] ][ $array_k[2] ];
					break;
				case 'd':
					$array_compara = $array_c[ $array_k[0] ][ $array_k[1] ][ $array_k[2] ][ $array_k[3] ];
					break;
				case 'e':
					$array_compara = $array_c[ $array_k[0] ][ $array_k[1] ][ $array_k[2] ][ $array_k[3] ][ $array_k[4] ];
					break;
				default:
					$array_compara = $array_c[ $array_k[0] ];
			}
			return $array_compara;
		}
		...
		echo "<td class='td_header td_h".($i)."' style='width:50px; border-right: 1px solid #CCCCCC;'>%</td>";
		echo	"</tr>";
											...
											if(is_array($e))
											{
												...
												foreach($e as $k_f => $f)
												{
													if(is_array($f))
													{
														...
														$v1 = $f['valor'];
														$total = ${$pos_total}['valor'];
														$porcentaje = number_format( (($v1*100)/$total), 2 ) ;
														
														$v1_c = $r_c[$k_a][$k_b][$k_c][$k_d][$k_e][$k_f]['valor'];
														
														//magia para hacer el paso de manera simple, intenté hacerlo de una manera mejor, pero no funcionó después sigo intentando.
														$array_tmp = get_array_compara( $r_c, array($k_a, $k_b, $k_c, $k_d, $k_e, $k_f), $pos_total ); 
														$total_c = $array_tmp['valor'];
