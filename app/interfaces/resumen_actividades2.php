...
<style>
	...
	.td_h1	{		<? if (sizeof($agrupadores) < 6)
	echo "display:none;" ?>	}
	.td_h2	{		<? if (sizeof($agrupadores) < 5)
	echo "display:none;" ?>	}
	.td_h3	{		<? if (sizeof($agrupadores) < 4)
	echo "display:none;" ?>	}
	.td_h4	{		<? if (sizeof($agrupadores) < 3)
	echo "display:none;" ?>	}
	.td_h5	{		<? if (sizeof($agrupadores) < 2)
	echo "display:none;" ?>	}
	...
	if (method_exists('Conf', 'GetConf') && Conf::GetConf($sesion, 'UsarAreaTrabajos')) {
		?>
								...
								if (UtilesApp::GetConf($sesion, 'UsoActividades')) {
									?>
									<option value="Actividades" <?= $tipo == 'Actividades' ? 'selected' : '' ?>><?= __('Actividades') ?></option>
								<?php
							}
							?>
							</select><br><br>
							...
							foreach ($c as $k_d => $d) {
								if (is_array($d)) {
									...
									if (array_key_exists('d', $campos_porcentajes)) {
										echo "<td class=\"tercer porcentaje\" rowspan=" . $d['filas'] . " > ";
									...
									foreach ($d as $k_e => $e) {
										if (is_array($e)) {
											...
											if (array_key_exists('e', $campos_porcentajes)) {
												echo "<td class=\"tercer porcentaje\" rowspan=" . $e['filas'] . " > ";
...
if ($opc == 'grafico') {
	...
	if ($nombres) {
		...
		foreach ($tiempos as $key => $tiempo) {
			if ($limite-- > 0)
				$datos_grafico .= "&nombres[]=" . $nombres[$key] . "&tiempo[]=" . str_replace(',', '.', $tiempos[$key]);
			else
				$otros += $tiempos[$key];
		}
		if ($otros)
