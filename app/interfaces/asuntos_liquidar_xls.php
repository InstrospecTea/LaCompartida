<?php
	...
	for($i=0; $i<$col_num; ++$i)
	{
		...
		if(in_array($col_name[$i],array('idcontrato'
										,'listado_codigo_asuntos'
										,'listado_codigo_asuntos'
										,'idmoneda'
										,'ultimo_id_cobro'
		...
		else if($col_name[$i] == 'glosa_ultimo_cobro') { $ws1->setColumn($arr_col[$col_name[$i]]['celda'], $arr_col[$col_name[$i]]['celda'], 25); }
		else if($col_name[$i] == 'ultimo_monto_cobro') { $ws1->setColumn($arr_col[$col_name[$i]]['celda'], $arr_col[$col_name[$i]]['celda'], 10); }
		else if($col_name[$i] == 'fecha_ultimo_cobro') { $ws1->setColumn($arr_col[$col_name[$i]]['celda'], $arr_col[$col_name[$i]]['celda'], 10); }
		else if($col_name[$i] == 'horas_castigadas') { $ws1->setColumn($arr_col[$col_name[$i]]['celda'], $arr_col[$col_name[$i]]['celda'], 10); }
		else if($col_name[$i] == 'hh_val_cobrado') { $ws1->setColumn($arr_col[$col_name[$i]]['celda'], $arr_col[$col_name[$i]]['celda'], 10); }
		else {$ws1->setColumn($arr_col[$col_name[$i]]['celda'], $arr_col[$col_name[$i]]['celda'], 10); }
		
		// css celdas
		if($col_name[$i] == 'hh_val_trabajo') { $arr_col[$col_name[$i]]['css'] = $formato_tiempo; }
		else if($col_name[$i] == 'horas_castigadas') { $arr_col[$col_name[$i]]['css'] = $formato_tiempo; }
		else if($col_name[$i] == 'fecha_ultimo_cobro') { $arr_col[$col_name[$i]]['css'] = $formato_fecha_tiempo; }
		...
		else if($col_name[$i] == 'glosa_ultimo_cobro') { $arr_col[$col_name[$i]]['titulo'] = __('Glosa Ult.').' '.__('Cobro'); }
		...
		for($i=0; $i<$col_num; $i++)
		{
	        if($arr_col[$col_name[$i]]['hidden'] != 'SI')
			{
				...
				else if($col_name[$i] == 'hh_val_cobrado')
				{
					...
					if($wip[0]=='0:00:00') {$hh_cobradas='';}
					else {
						$hh_cobradas = ($wip[0]/24); //Excel calcula el tiempo en días
				...
				{
					$wip = $contrato->ProximoCobroEstimado('',Utiles::fecha2sql($proc->fields['fecha_ultimo_cobro']), $proc->fields['idcontrato'], true);
					if($wip[2]=='0:00:00') {$hh_castigadas='';}
					else {
						$hh_castigadas = ($wip[2]/24); //Excel calcula el tiempo en días
