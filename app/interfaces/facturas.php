...
<script type="text/javascript">
	...
	function mensajeEditarFactura(estado, nCobro, glosaDocLegal) {
		var mensaje = "<?php echo __('El cobro %num asociado'); ?>".replace('%num', nCobro) + " a esta " + glosaDocLegal + " está en estado "
									+ estado + ", debe emitir <?php echo __('el cobro antes indicado'); ?> para poder editar esta " + glosaDocLegal + "";
...
function Opciones($fila) {
	...
	if ($fila->fields['estado_cobro'] == 'EN REVISION' || $fila->fields['estado_cobro'] == 'CREADO') {
		$boton_editar = '<a class="fl ui-button editar_off" href="javascript:void(0);" style="margin: 3px 1px;width: 18px;height: 18px;" onclick="mensajeEditarFactura(\'' .
											$fila->fields['estado_cobro'] . '\', \'' . $fila->fields['id_cobro'] . '\', \'' . $fila->fields['glosa_doc_legal'] . '\');" title="Editar Factura" disabled></a>';
	} else {
