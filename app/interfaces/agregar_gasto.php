...
<form method=post action="<?php echo $SERVER[PHP_SELF] ?>"  id="form_gastos" autocomplete='off'>
	...
	<table class="border_plomo" style="background-color: #FFFFFF;" width='90%'>
		<tr>
			...
			<td align=left>
				<input type="text" name="fecha" value="<?php echo $gasto->fields[fecha] ? Utiles::sql2date($gasto->fields[fecha]) : date('d-m-Y') ?>" id="fecha" size="11" maxlength="10" />
		...
		<tr>
			...
			<td align=left>
				...
				if (UtilesApp::GetConf($sesion, 'TipoSelectCliente') == 'autocompletador') {
					...
					$input_id_codigo_cliente = "codigo_cliente";
					$input_id_codigo_asunto = "codigo_asunto";
					$input_id_valor_cliente = $gasto->fields['codigo_cliente'] ? $gasto->fields['codigo_cliente'] : $codigo_cliente;

					if (UtilesApp::GetConf($sesion, 'CodigoSecundario')) {
						$input_id_codigo_cliente = "codigo_cliente_secundario";
						$input_id_codigo_asunto = "codigo_asunto_secundario";
						$input_id_valor_cliente = $codigo_cliente_secundario;
					}

					InputId::Imprimir($sesion, "cliente", $input_id_codigo_cliente, "glosa_cliente", $input_id_codigo_cliente, $input_id_valor_cliente, "", "CargarSelect('$input_id_codigo_cliente','$input_id_codigo_asunto','cargar_asuntos');", 320);
				}
				?>
		...
		<tr>
			...
			<td align=left>
				<?php
				$input_id_codigo_asunto = "codigo_asunto";
				$input_id_valor_asunto = $gasto->fields['codigo_asunto'] ? $gasto->fields['codigo_asunto'] : $codigo_asunto;
				$input_id_valor_cliente = $gasto->fields['codigo_cliente'] ? $gasto->fields['codigo_cliente'] : $codigo_cliente;

				if (UtilesApp::GetConf($sesion, 'CodigoSecundario')) {
					$input_id_codigo_asunto = "codigo_asunto_secundario";
					$input_id_valor_asunto = $codigo_asunto_secundario;
					$input_id_valor_cliente = $codigo_cliente_secundario;
				}

				echo InputId::Imprimir($sesion, "asunto", $input_id_codigo_asunto, "glosa_asunto", $input_id_codigo_asunto, $input_id_valor_asunto, "", "CargaIdioma(this.value);CargarSelectCliente(this.value, 'gastos');", 320, $input_id_valor_cliente);
				?>
				<span style="color:#FF0000; font-size:10px">*</span>
			</td>
		</tr>
		...
		<?php if (UtilesApp::GetConf($sesion, 'FacturaAsociada')) { ?>
			...
			<tr>
				...
				<td>
					<input type="text" name="fecha_factura_asociada" value="<?php echo ($gasto->fields['fecha_factura'] && $gasto->fields['fecha_factura'] != 'NULL') ? Utiles::sql2date($gasto->fields['fecha_factura']) : '' ?>" id="fecha_factura_asociada" size="11" maxlength="10" />
					<img src="<?php echo Conf::ImgDir() ?>/calendar.gif" id="img_fecha_factura_asociada" style="cursor:pointer" />
				</td>
			</tr>
		<?php } ?>
			...
			<div id='tabla_gastos'>
				<table style="border: 1px solid black;" width='90%'>
					<tr>
						...
						<td align=left>
							<input type="text" name="fecha_pago" value="<?php echo $ingreso->fields['fecha'] ? Utiles::sql2date($ingreso->fields['fecha']) : date('d-m-Y') ?>" id="fecha_pago" size="11" maxlength="10" />
...
<?php if (UtilesApp::IdiomaGrande() && $codigo_asunto) { ?>
