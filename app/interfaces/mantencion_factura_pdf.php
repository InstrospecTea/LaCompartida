...
if (empty($id_estudio)) {
	$id_estudio = $estudios_array[0]['id_estudio'];
}
...
<script type="text/javascript">
	...
	jQuery(document).ready(function () {
		var Id_estudio = jQuery('#select_id_estudio').val();
		...
		jQuery('#select_id_estudio').change(function () {
			actualizarDocumento();
		});
		...
		jQuery('.facstyle').live('change', function () {
			...
			var Css = '';
			if (Style == '')
				Css = {'text-decoration': 'none', 'font-weight': 'normal', 'font-style': 'normal'};
			if (Style == 'B')
				Css = {'text-decoration': 'none', 'font-weight': 'bold', 'font-style': 'normal'};
			if (Style == 'I')
				Css = {'text-decoration': 'none', 'font-weight': 'normal', 'font-style': 'italics'};
			if (Style == 'U')
				Css = {'text-decoration': 'underline', 'font-weight': 'normal', 'font-style': 'normal'};
			jQuery('#caja_' + ID).css(Css);
		});

		jQuery('.facalign').live('change', function () {
			var ID = jQuery(this).attr('rel');
			var Style = jQuery('#fac_align_' + ID).val();
			var Css = '';
			if (Style == 'L')
				Css = {'text-decoration': 'none', 'text-align': 'left'};
			if (Style == 'R')
				Css = {'text-decoration': 'none', 'text-align': 'right'};
			if (Style == 'C')
				Css = {'text-decoration': 'none', 'text-align': 'center'};
			if (Style == 'J')
				Css = {'text-decoration': 'none', 'text-align': 'justify'};
			jQuery('#caja_' + ID).css(Css);
		});
	...
	function agregaporid(ID, Cat) {
		...
		if (Relleno.length == 0)
			Relleno = jQuery('#glosa_' + ID).attr('rel');
		if (Relleno.length == 0)
			Relleno = jQuery('#glosa_' + ID).html();
		...
		if (Height == 0)
			Height = Fontsize * (ArrayRelleno.length) / 1.8;
		Height = Height * factor;
		var Font = jQuery('#fac_font_' + ID).val();
		var Style = jQuery('#fac_style_' + ID).val();
		var Align = jQuery('#fac_align_' + ID).val();
		var Mayus = jQuery('#fac_mayuscula_' + ID).val();
		var transform = 'none';
	...
	function YoucangonowMichael() {
		Id_estudio = jQuery('#select_id_estudio').val();
		...
		jQuery.post(
						'ajax/mantencion_factura_pdf_ajax.php',
						{
							opc: 'dibuja_tabla',
		...
		jQuery.when(jQuery.ajax({async: false, cache: true, type: "GET", url: 'https://ajax.googleapis.com/ajax/libs/swfobject/2.2/swfobject.js', dataType: 'script'}),
						jQuery.ajax({async: false, cache: true, type: "GET", url: staticpath + 'jquery.uploadify.v2.1.0.min.js', dataType: 'script'})).then(function () {
...
<?php echo __('Tipo documento legal:'); ?>&nbsp;
		...
		<td align="left">
<?php echo Html::SelectQuery($sesion, "SELECT id_documento_legal, glosa FROM prm_documento_legal", "select_id_documento_legal", $id_documento_legal, 'style="width:100px;"'); ?>
		</td>
		<td style="text-align:right;vertical-align: middle;">&nbsp;&nbsp;
...
<style type="text/css">
	.cabecerax  {border:0 none; position:relative; width: 1000px;height:35px;}
	.cabecerax ul {display:block;width:1000px;height:35px;margin:auto;list-style:none;margin:0 0 0 0;color:white;}
	.cabecerax ul li {border:1px solid #42A62B;float:left;height:35px;display:inline-block;background-color:#42A62B;text-align:center;vertical-align:middle;}

	.cabecerax li.st1cellx {width:150px;text-align:center;}
	.cabecerax li.nd2cellx {width:30px;text-align:center;}
	.cabecerax li.rd3cellx, td.rd3cellx {width:60px;text-align:center;}
	.cabecerax li.th4cellx {width:60px;text-align:center;}
	.cabecerax li.th5cellx {width:60px;text-align:center;}
	.cabecerax li.th6cellx {width:60px;text-align:center;}
	.cabecerax li.th7cellx {width:120px;text-align:center;}
	.cabecerax li.th8cellx {width:80px;text-align:center;}
	.cabecerax li.th9cellx {width:90px;text-align:center;}
	.cabecerax li.th10cellx {width:50px;text-align:center;}
	.cabecerax li.th11cellx {width:50px;text-align:center;}

	#contienecoordenadas {border:0 none; position:relative; overflow-y: hidden;overflow-x:hidden; width: 1100px;margin:5px 0;}

	#tabla_coordenadas ul {display:block;width:880px;height:24px;float:left;list-style:none;margin:0 0 0 0;}

	#tabla_coordenadas ul li  {border:1px solid transparent;float:left;height:22px;display:inline-block;}
	#tabla_coordenadas {position:relative;margin:0 0 0 8px;float:left;top:0;width:800px;}
	#tabla_coordenadas li.st1cellx  {width:150px;text-align:left;clear:left !important;}
	#tabla_coordenadas li.nd2cellx {width:30px;text-align:center;}
	...
	#tabla_coordenadas li.th6cellx {width:60px;text-align:center;}
	#tabla_coordenadas li.th7cellx {width:120px;text-align:center;}
	#tabla_coordenadas li.th8cellx {width:85px;text-align:center;}
	#tabla_coordenadas li.th9cellx {width:85px;text-align:center;}
	#tabla_coordenadas li.th10cellx {width:80px;text-align:center;}
	#tabla_coordenadas li.th10cellx  input {width:30px;}
