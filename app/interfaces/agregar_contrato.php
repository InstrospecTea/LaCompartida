...
<<<<<<< HEAD
$rango1 = ( $contrato->escalonadas[1]['tiempo_inicial'] ? $contrato->escalonadas[1]['tiempo_inicial'] : '0' ) . ' - ' . ( $contrato->escalonadas[1]['tiempo_final'] ? $contrato->escalonadas[1]['tiempo_final'] : '' );
$rango2 = ( $contrato->escalonadas[2]['tiempo_inicial'] ? $contrato->escalonadas[2]['tiempo_inicial'] : '0' ) . ' - ' . ( $contrato->escalonadas[2]['tiempo_final'] ? $contrato->escalonadas[2]['tiempo_final'] : '' );
$rango3 = ( $contrato->escalonadas[3]['tiempo_inicial'] ? $contrato->escalonadas[3]['tiempo_inicial'] : '0' ) . ' - ' . ( $contrato->escalonadas[3]['tiempo_final'] ? $contrato->escalonadas[3]['tiempo_final'] : '' );

$ultimo_rango = ( $contrato->escalonadas[$contrato->escalonadas['num']]['tiempo_inicial'] ?	$contrato->escalonadas[$contrato->escalonadas['num']]['tiempo_inicial'] : '0' ) . ' - ' . ( $contrato->escalonadas[$contrato->escalonadas['num']]['tiempo_final'] ?	$contrato->escalonadas[$contrato->escalonadas['num']]['tiempo_final'] : '' );

//	aqui es popup de contrato directo agregar.
$div_show = ($popup && !$motivo); 

$query_count = "SELECT COUNT(usuario.id_usuario) FROM usuario JOIN usuario_permiso USING(id_usuario) WHERE codigo_permiso='SOC'";
...
<?php } ?>
	...
	<table id="responsable">
		...
		<tr class="controls controls-row ">
			...
			<td class="al">
				<label for="activo_contrato" class="inline-help">
					...
					&nbsp;<?php echo __('Los contratos inactivos no aparecen en el listado de cobranza.') ?>
				</label>
			</td>
		</tr>
		...
		if (UtilesApp::GetConf($Sesion, 'UsarImpuestoSeparado')) {
			...
			echo '<tr class="controls controls-row ">';
				...
				echo '<td class="al">';
					...
					echo '<input type="hidden" name="usa_impuesto_separado" value="0"/>';
					echo '<input class="span1" type="checkbox" name="usa_impuesto_separado" id="usa_impuesto_separado" value="1" ' .$chk .' />';
				echo '</td>';
		...
		if (UtilesApp::GetConf($Sesion, 'UsarImpuestoPorGastos')) {
			echo '<tr class="controls controls-row ">';
				...
				echo '</td>';
				echo '<td class="al">';
					echo '<input type="hidden" name="usa_impuesto_gastos" value="0"/>';
		...
		<tr class="controls controls-row ">
			<td class="al">
				<div class="span4">
					<?php echo __('Liquidar por separado (honorario y gastos)') ?>
				</div>
			</td>
			<td class="al">
				<div class="span1">
...
>>>>>>> 13.7.37
		...
		<?php if ($modulo_retribuciones_activo) {
			...
			$usuario_responsable_retrubucion = empty($contrato->fields['id_contrato']) ? UtilesApp::GetConf($Sesion, 'RetribucionUsuarioResponsable') : $contrato->fields['retribucion_usuario_responsable'];
			...
			echo '<tr>';
				...
				echo '</td>';
				echo '<td class="al">';
					echo '<input name="retribucion_usuario_responsable" type="text" size="6" value="'. $usuario_responsable_retrubucion .'"/>%';
				echo '</td>';
			echo '</tr>';
		...
		<?php  if (UtilesApp::GetConf($Sesion, 'EncargadoSecundario')) {
			...
			$query = "SELECT usuario.id_usuario,CONCAT_WS(' ',apellido1,apellido2,',',nombre) FROM usuario WHERE activo = 1 OR id_usuario = '" . $contrato->fields['id_usuario_secundario'] . "'ORDER BY apellido1";
		...
		<?php if ($modulo_retribuciones_activo) { ?>
			...
			<tr>
				...
				<td class="al">
					<input name="retribucion_usuario_secundario" type="text" size="6" value="<?php echo empty($contrato->fields['id_contrato']) ? UtilesApp::GetConf($Sesion, 'RetribucionUsuarioSecundario') : $contrato->fields['retribucion_usuario_secundario']; ?>" />%
				</td>
			</tr>
		...
		<?php } if (UtilesApp::GetConf($Sesion, 'ExportacionLedes')) { ?>
			<tr class="controls controls-row ">
				...
				<td class="al">
					<input type="hidden" name="exportacion_ledes" value="0"/>
	...
	<?php if (UtilesApp::GetConf($Sesion, 'SetFormatoRut')) {
	...
	<?php echo!$div_show ? '<span id="datos_factura_img"><img src="' . Conf::ImgDir() . '/mas.gif" border="0" id="datos_factura_img"></span>' : '' ?>&nbsp;<?php echo __('Datos Facturación') ?></legend>
	...
	<table id='datos_factura' style='display:<?php echo $show ?>'>
		...
		<tr>
			...
			<td align="left" colspan="5">
				<input type="text" class="span1" name='cod_factura_telefono' size=8 value="<?php echo $contrato->fields['cod_factura_telefono'] ?>" />&nbsp;
				<input type="text" class="span2" name='factura_telefono' size=30 value="<?php echo $contrato->fields['factura_telefono'] ?>" />
			</td>
		</tr>
		...
		<tr>
			...
			<td align="left" colspan="2">
				...
				echo Html::SelectQuery($Sesion, $query_cuenta_banco, "id_cuenta", $contrato->fields['id_cuenta'] ? $contrato->fields['id_cuenta'] : $id_cuenta, '', $tiene_banco ? "" : "Cualquiera", "150");
				?>
			</td>
		</tr>
		...
		if (UtilesApp::GetConf($Sesion, 'SegundaCuentaBancaria')) {
			...
			echo '<tr>';
				...
				echo '<td align="left" colspan="2">';
					echo Html::SelectQuery($Sesion, "SELECT id_banco, nombre FROM prm_banco ORDER BY orden", "id_banco2", $id_banco2, 'onchange="CargarCuenta(\'id_banco2\',\'id_cuenta2\');"', "Cualquiera", "150");
				echo '</td>';
				echo '<td align="right" colspan="1">';
					echo __('Cuenta Secundaria');
				echo '</td>';
				echo '<td align="left" colspan="2">';
					...
					echo Html::SelectQuery($Sesion, $query_cuenta_banco, "id_cuenta2", $contrato->fields['id_cuenta2'] ? $contrato->fields['id_cuenta2'] : $id_cuenta2, '', $tiene_banco ? "" : "Cualquiera", "150");
				echo '</td>';
			echo '</tr>';
		} ?>
			...
			if (count($estudios_array) > 1) {
				echo '<tr>';
					...
					echo '<td align="left" colspan="5">';
						echo Html::SelectArray($estudios_array, "id_estudio", $id_estudio);
					echo '</td>';
				echo '</tr>';
			} else {
...
=======
					var cliente = '&codigo_cliente='+$('<?php echo Conf::GetConf($Sesion, 'CodigoSecundario') ? 'codigo_cliente_secundario' : 'codigo_cliente'; ?>').value;
>>>>>>> 13.7.37
							...
							<table style='border: 0px solid' bgcolor='#C6DEAD'>
								<?php if ($cobro) { ?>
									<tr>
										...
										<td align="left">
											&nbsp;<label style='background-color:#FFFFFF'><?php echo $cobro->TotalCobrosCap($contrato->fields['id_contrato']) > 0 ? $cobro->TotalCobrosCap($contrato->fields['id_contrato']) : 0; ?> </label>
										</td>
									</tr>
								<?php } ?>
								<tr>
...
>>>>>>> 13.7.37
	...
	<table   id='datos_carta' style='display:<?php echo $show ?>' width="100%">
		...
		<tr>
			...
			<td align="left" colspan='5'>
				<?php if ($contrato->fields['opc_papel'] == '' && UtilesApp::GetConf($Sesion, 'PapelPorDefecto')) {
...
>>>>>>> 13.7.37
				?>
					<input type="hidden" name="opc_ver_solicitante" value="0"/><input type="checkbox" name="opc_ver_solicitante"  value="1" <?php echo $contrato->fields['opc_ver_solicitante'] == '1' ? 'checked="checked"' : '' ?>>
				<?php
<<<<<<< HEAD
				echo '</td>';
				echo '<td align="left" colspan="5">';
					echo '<label>';
						echo __('Mostrar solicitante'); 
					echo '</label>';
				echo '</td>';
			echo '</tr>';
		...
		<?php if (Conf::GetConf($Sesion, 'ResumenProfesionalVial') ) { ?>
			<tr>
				<td align="right" colspan='1'>
					...
					<input type="checkbox" name="opc_restar_retainer"  value="1" <?php echo $contrato->fields['opc_restar_retainer'] == '1' ? 'checked="checked"' : '' ?> />
				</td>
				<td align="left" colspan='5'>
					<label><?php echo __('Restar valor retainer') ?></label>
				</td>
			</tr>
<<<<<<< HEAD
			<tr>
				<td align="right">
					...
					<input type="checkbox" name="opc_ver_detalle_retainer"  value="1" <?php echo $contrato->fields['opc_ver_detalle_retainer'] == '1' ? 'checked="checked"' : '' ?> />
				</td>
				<td align="left" colspan='5'>
		...
		<?php } else { ?>
			...
			<tr>	
				...
				<td>
					<input type="hidden" id="opc_ver_detalle_retainer" name="opc_ver_detalle_retainer" value="1"/>
=======
			<?php
...
<<<<<<< HEAD
	...
	<?php if ($id_cliente || $id_asunto) { ?>
		...
		<table id='documentos' style='display:<?php echo $show ?>'>
			<tr>
				<td colspan="2" align="center">
					<?php $id_contrato_ifr = $contrato->fields['id_contrato']; ?>
...
=======
					...
					<!-- FIN RESPONSABLE -->
					<?php
					if (Conf::GetConf($Sesion, 'SetFormatoRut'))
						$setformato = "SetFormatoRut();";
					else
						$setformato = "";
					?>
...
>>>>>>> 13.7.37
	...
	function YoucangonowMichael() {
		...
		<?php if (Conf::GetConf($Sesion, "CopiarEncargadoAlAsunto") && !$desde_agrega_cliente) {
			if (Conf::GetConf($Sesion, 'EncargadoSecundario')) {
...
>>>>>>> 13.7.37
		...
		<?php if ($validaciones_segun_config) { ?>
			...
			if (!$$('[name="forma_cobro"]') {
				.any(function(elem){return elem.checked;})) {
...
>>>>>>> 13.7.37
	...
	<?php // numeros de cobros existentes para ver cual sigue
