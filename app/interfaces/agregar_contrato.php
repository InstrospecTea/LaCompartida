...
<?php } ?>
		...
		<?php $query = UsuarioExt::QueryComerciales(); ?>

		<tr class="controls controls-row ">
			<td class="al">
				<div class="span4">
					<?php echo __('Encargado Comercial') . $obligatorio; ?>
				</div>
			</td>
			<td class="al">
				<?php
			...
			echo '<tr class="controls controls-row">';
				echo '<td class="al">';
					echo '<div class="span4">';
						echo __('Encargado Secundario') . $obligatorio;
					echo '</div>';
				echo '</td>';
				echo '<td class="al">';
					echo Html::SelectQuery($Sesion, $query, "id_usuario_secundario", $contrato->fields['id_usuario_secundario'] ? $contrato->fields['id_usuario_secundario'] : '', " class='span3' ", "Vacio", "200");
				echo '</td>';
			echo '</tr>';
	...
	<fieldset style="width: 97%;background-color: #FFFFFF; margin-top: 20px;">
		...
		<table id='datos_factura' style='display:<?php echo $show ?>'>
			...
			<?php if (Conf::GetConf($Sesion, 'SegundaCuentaBancaria')) {
	...
	<fieldset style="width: 97%; background-color: #FFFFFF;">
		<legend <?php echo!$div_show ? 'onClick="MuestraOculta(\'datos_solicitante\')" style="cursor:pointer"' : '' ?> > <?php echo!$div_show ? '<span id="datos_solicitante_img"><img src="' . Conf::ImgDir() . '/mas.gif" border="0" id="datos_solicitante_img"></span>' : '' ?> &nbsp;<?php echo __('Solicitante') ?></legend>
	...
	<fieldset style="width: 98%; background-color: #FFFFFF;">
		<legend <?php echo!$div_show ? 'onClick="MuestraOculta(\'datos_cobranza\')" style="cursor:pointer"' : '' ?> /> <?php echo!$div_show ? '<span id="datos_cobranza_img"><img src="' . Conf::ImgDir() . '/mas.gif" border="0" id="datos_cobranza_img"></span>' : '' ?> &nbsp;<?php echo __('Datos de Tarificación') ?> </legend>
						...
						<table  style="float:left;" class="span7">
							...
							<tr>
								<td class="span4">
									...
									<div class="controls controls-row ">
										<label for="tipo_tarifa_flat"  class="span2" >	<input type="radio"  name="tipo_tarifa" id="tipo_tarifa_flat" value="flat" <?php echo empty($valor_tarifa_flat) ? '' : 'checked' ?>/> Plana por </label>
					...
					<td colspan="2">
						<div style='border:1px solid #999999;width:400px;padding:4px 4px 4px 4px' id="div_forma_cobro">
							<div id="div_monto" align="left" style="display:none; background-color:#C6DEAD;padding-left:2px;padding-top:2px;">
								<span id="span_monto">&nbsp;<?php echo __('Monto') . $obligatorio ?>
								...
								&AElig;&nbsp;&nbsp;<?php echo __('Moneda') . $obligatorio ?>
				...
				<?php echo __('Descuento') ?>
					...
					<td align="left">
						<div   class="controls controls-row ">
							...
							<div class="span1"> 
								<input type=radio name=tipo_descuento id=tipo_descuento value='VALOR' <?php echo $contrato->fields['tipo_descuento'] == 'VALOR' ? 'checked="checked"' : '' ?> /><span class="inline-help"><?php echo __('Valor') ?></span>
							</div>
						</div>
						<br>
						<div   class="controls controls-row ">
							...
							<div class="span1"> 
								<input type=radio name=tipo_descuento id=tipo_descuento value='PORCENTAJE' <?php echo $contrato->fields['tipo_descuento'] == 'PORCENTAJE' ? 'checked="checked"' : '' ?> /> <span class="inline-help">  <?php echo __('%') ?></span>
							</div>
						</div>
					</td>
				</tr>
			...
			<table width="80%" border="0" style="border: 1px solid #BDBDBD;" cellpadding="3" cellspacing="3" id="user_generators_form">
				<tbody>
					<tr>
						...
						<td>
							<?php
							$query_usuario_generador = "SELECT usuario.id_usuario,CONCAT_WS(' ',apellido1,apellido2,',',nombre)
															FROM usuario
																WHERE usuario.id_usuario IN (SELECT id_usuario FROM usuario_permiso)
																	AND usuario.activo = 1
																		ORDER BY usuario.apellido1";
							echo Html::SelectQuery($Sesion, $query_usuario_generador, "id_user_generator", "", "  ", "Seleccione", "200");
							?>
						</td>
						<td>
							Porcentaje Genera:
