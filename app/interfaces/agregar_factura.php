	...
	if ($desde_webservice) { //El webservice ignora todo llamado a $pagina
		...
		if (UtilesApp::GetConf($sesion, 'NuevoModuloFactura')) {
			...
			if (UtilesApp::GetConf($sesion, 'UsarGastosConSinImpuesto')) {
				if (!is_numeric($monto_gastos_sin_iva)) {
...
<?php if (UtilesApp::GetConf($sesion, 'NuevoModuloFactura')) { ?>
	...
	<?php if (UtilesApp::GetConf($sesion, 'UsarGastosConSinImpuesto') == '1') { ?>
...
<?php } else if (UtilesApp::GetConf($sesion, 'CodigoSecundario')) { ?>
...
<?php if (UtilesApp::GetConf($sesion, 'NuevoModuloFactura')) { ?>
	...
	<?php if (!$factura->loaded() && ($id_documento_legal != 2)) { ?>
				...
				if ((form.id_documento_legal.value!=2) && 
					(saltar_validacion_saldo==0) && 
	...
	<?php } ?>
<?php } else { // Sin nuevo modulo facturacion  ?>
...
if (!$factura->loaded() && $id_cobro && $id_documento_legal != 2) {
	...
	if ($saldo > 0) {
		?>
				if (confirm('<?php echo__('Existen adelantos por ') . $simbolo . ' ' . number_format($saldo, $cifras_decimales) . __(' asociados a esta liquidación. ¿Desea utilizarlos para saldar esta ') . $tipo_documento_legal . '?'; ?>')) {
...
if ($id_cobro > 0) {
	...
	if ($cobro->fields['porcentaje_impuesto_gastos'] == 0 &&
			(UtilesApp::GetConf($sesion, 'ValorImpuestoGastos'))) {
		?>
...
<?php if (UtilesApp::GetConf($sesion, 'CantidadDecimalesTotalFactura') != '-1') { ?>
...
<form method=post id="form_facturas" name="form_facturas">
	...
	<table width='90%'>
		<tr>
			<td align=left>
				<strong><?php echo $txt_pagina ?></strong>
			</td>
		</tr>
	</table>
	<br />
	<table style="border: 0px solid black;" width='90%'>
		<tr>
			<td align=left>
				<strong><?php echo __('Información de') . ' ' . $tipo_documento_legal; ?></strong>
			</td>
		</tr>
	</table>
	<?php
...
//Se debe elegir un documento legal padre si:
		...
		<tr>
			...
			<td>
				<span style='display:none' id=letra_inicial>&nbsp;&nbsp;
					<?php echo __('Letra'); ?>
					:&nbsp;
...
<!--<input type="text" name="cliente" value="<?php echo $factura->fields['cliente']; ?>" id="cliente" size="70" maxlength="99" />-->
		...
		if (UtilesApp::GetConf($sesion, 'NuevoModuloFactura')) {
			...
			<tr>
				...
				<td align="left">
					<?php if (UtilesApp::GetConf($sesion, 'DescripcionFacturaConAsuntos')) { ?>
					...
					<?php } else if ($cantidad_lineas_descripcion > 1) { ?>
						<textarea name="descripcion_honorarios_legales" cols="50" class="disable_paste" onkeydown="return RestriccionGlosaFactura(event, this);" rows="<?php echo$cantidad_lineas_descripcion ?>" style="font-family: Arial; font-size: 11px; text-align: left;"><?php echo $descripcion_honorario; ?></textarea>
					<?php } else { ?>
						<input type="text" name="descripcion_honorarios_legales" id="descripcion_honorarios_legales" value="<?php echo $descripcion_honorario; ?>" maxlength="300" size="40" />
					<?php } ?>
				</td>
			...
			<tr>
				...
				<td align=left>
						<!--<input type="text" name="descripcion_gastos_con_iva" value="<?php echo $descripcion_subtotal_gastos; ?>" size="40" maxlength="30">-->
					<?php if ($cantidad_lineas_descripcion > 1) { ?>
						<textarea name="descripcion_gastos_con_iva" cols="50" class="disable_paste" onkeydown="return RestriccionGlosaFactura(event, this);" rows="<?php echo$cantidad_lineas_descripcion ?>" style="font-family: Arial; font-size: 11px; text-align: left;"><?php echo $descripcion_subtotal_gastos; ?></textarea>
					<?php } else { ?>
						<input type="text" name="descripcion_gastos_con_iva" value="<?php echo$descripcion_subtotal_gastos; ?>" size="40" maxlength="30">
					<?php } ?>
				</td>
			...
			<?php if (UtilesApp::GetConf($sesion, 'UsarGastosConSinImpuesto') == '1') { ?>
				<tr>
					...
					<td align=left>
							<!--<input type="text" name="descripcion_gastos_sin_iva" id="descripcion_gastos_sin_iva" value="<?php echo $descripcion_subtotal_gastos_sin_impuesto; ?>" size="40" maxlength="30" onkeydown="MontoValido( this.id );">-->
						<?php if ($cantidad_lineas_descripcion > 1) { ?>
							<textarea name="descripcion_gastos_sin_iva" cols="50" class="disable_paste" onkeydown="return RestriccionGlosaFactura(event, this);" rows="<?php echo$cantidad_lineas_descripcion ?>" style="font-family: Arial; font-size: 11px; text-align: left;"><?php echo $descripcion_subtotal_gastos_sin_impuesto; ?></textarea>
						<?php } else { ?>
							<input type="text" name="descripcion_gastos_sin_iva" id="descripcion_gastos_sin_iva" value="<?php echo $descripcion_subtotal_gastos_sin_impuesto; ?>" size="40" maxlength="30" onkeydown="MontoValido( this.id );">
						<?php } ?>
					</td>
		...
		<tr>
			<td colspan="4" align=center>
				...
				<a href='javascript:void(0)' onclick="MostrarTipoCambioPago()" title="<?php echo __('Tipo de Cambio del Documento de Pago al ser pagado.') ?>">
		...
		<tr>
			...
			WHERE id_cobro = '" . $id_cobro . "'";
									...
									while (list($id_moneda, $glosa_moneda, $tipo_cambio) = mysql_fetch_array($resp)) {
										?>
										<td>
											<span><strong><?php echo $glosa_moneda; ?></strong></span><br />
								...
								<tr>
									<td colspan=<?php echo $num_monedas; ?> align=center><input type=button onclick="ActualizarDocumentoMonedaPago($('todo_cobro'))" value="<?php echo __('Guardar'); ?>" />
										...
										<input type="hidden" id="ids_monedas_factura" name="ids_monedas_factura" value="<?php echo implode(',', $ids_monedas); ?>" />
									</td>
								</tr>
							</table>
						</div>
