...
if (Conf::EsAmbientePrueba()) {
	$query = "UPDATE contrato SET usa_impuesto_separado = '" . Conf::GetConf($sesion,'UsarImpuestoSeparado') . "', "
	...
	echo "Fecha Inicio: $fecha_inicio, mktime: $mktime_inicio, dia: $dia_inicio, mes: $mes_inicio, año: $anio_inicio<br />";
	echo "Fecha Fin: $fecha_fin, mktime: $mktime_fin, dia: $dia_fin, mes: $mes_fin, año: $anio_fin<br />";

	$acciones = array(
		'Conversación con',
		'Escribir correo electronico para',
		'Reunión con',
		'Almuerzo con',
		'Escribir correo electronico para');
	$personas = array(
				...
				while (Utiles::time2decimal($duracion_en_este_dia) < $horas_maximas && $cont_trabajos_total < ( $max_dia + 4 )) {
					if (rand(1,100) < 66 && $cont_trabajos < $max_dia) {
						...
						$query = "INSERT INTO trabajo
												(id_moneda,fecha,codigo_asunto,descripcion,duracion,duracion_cobrada,id_usuario)
	...
	while ($fecha <= $fecha_mk_fin) {
		...
		for ($j = 0; $j < count($asuntos); $j++) {
			...
			while ($cont_gastos < $max_mes) {
				...
				$query = "INSERT INTO cta_corriente
										( codigo_cliente, codigo_asunto, fecha, id_moneda, ingreso, egreso, monto_cobrable, descripcion )
