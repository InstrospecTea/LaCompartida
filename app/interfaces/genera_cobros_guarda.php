...
if ($tipo_liquidacion) {
	 //1:honorarios, 2:gastos, 3:mixtas
...
if ($print || $emitir || $en_revision) {
	...
	$url = "genera_cobros.php?activo=$activo&id_usuario=$id_usuario&codigo_cliente=$codigo_cliente&fecha_ini=$fecha_ini" . "&fecha_fin=$fecha_fin&opc=buscar&rango=$rango&fecha_anio=$fecha_anio&fecha_mes=$fecha_mes&fecha_periodo_ini=$fecha_periodo_ini" . "&fecha_periodo_fin=$fecha_periodo_fin&usar_periodo=$usar_periodo&tipo_liquidacion=$tipo_liquidacion&forma_cobro=$forma_cobro&codigo_asunto=$codigo_asunto";
}
...
if ($print) {
	...
	while ($cobro = mysql_fetch_array($resp)) {
		...
		if ($Cobro->Load($cobro['id_cobro'])) {
			...
			if (empty($retorno_guardar_cobro)) {
			...
			array_push($errores_cobro, utf8_encode("#{$cobro['id_cobro']}: No existe cobro"));
		}
	}
