...
$query = "SELECT 
			...
			FROM (
				...
				LEFT JOIN 
					(SELECT SUM( TIME_TO_SEC( $duracion_query ) ) /3600 AS duracion, DATE_FORMAT( trabajo.fecha,  '%Y-%m-01' ) AS periodo
						FROM trabajo
...
if ($tipo_duracion == 'no_cobrable') {
	$barLayerObj = $c->addBarLayer3($duracion, $color, __("Horas no cobrables") . ': ' . $total_duracion . ' en ' . $i . ' meses', 2);
	$barLayerObj->setBorderColor(-1, 1);
	$barLayerObj->setAggregateLabelStyle("arialbd.ttf", 8, 0x000);
}
if ($tipo_duracion == 'cobrada') {
	$barLayerObj = $c->addBarLayer3($duracion, $color, __("Horas No cobradas") . ': ' . $total_duracion . ' en ' . $i . ' meses', 2);
	$barLayerObj->setBorderColor(-1, 1);
	$barLayerObj->setAggregateLabelStyle("arialbd.ttf", 8, 0x000);
}
