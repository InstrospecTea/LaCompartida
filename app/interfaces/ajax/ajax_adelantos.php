...
if ($_REQUEST['accion'] == 'listaadelanto') {
	...
	$query .= isset($_GET['eliminados']) ? 'es_adelanto=-1' : 'es_adelanto=1';
	if (isset($_GET['tiene_saldo']) && $_GET['tiene_saldo'] == 1) {
		$query .= " AND saldo_pago < 0 ";
	}
	if (!empty($_GET['id_documento'])) {
		$query .= " AND documento.id_documento = " . intval($_GET['id_documento']);
	}
	if (!empty($_GET['campo_codigo_asunto'])) {
		$query .= " AND asuntos.codigo_asuntos like '%" . $_GET['campo_codigo_asunto'] . "%'";
	}
	if (!empty($_GET['codigo_cliente'])) {
		$query .= " AND cliente.codigo_cliente = '" . $_GET['codigo_cliente'] . "' ";
	}
	if (!empty($_GET['id_contrato'])) {
		$query .= " AND (documento.id_contrato = '" . intval($_GET['id_contrato']) . "' OR documento.id_contrato IS NULL)";
	}

	if (!empty($_GET['fecha1'])) {
		$query .= " AND documento.fecha >= " . formatofecha($_GET['fecha1']);
	}
	if (!empty($_GET['fecha2'])) {
		$query .= " AND documento.fecha <=  " . formatofecha($_GET['fecha2']);
	}
	if (!empty($_GET['moneda_adelanto'])) {
		$query .= " AND documento.id_moneda = " . intval($_GET['moneda']);
	}
