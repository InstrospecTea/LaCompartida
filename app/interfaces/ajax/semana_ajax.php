...
<?php  $arraytrabajo=array();
	...
	$dia_anterior=2;
	$total[0]=$dia_semana=0;
	echo("<div class='celdadias' id='celdadia".$dia_anterior."'>");  
...
#		$total[$dia_semana] += ($alto/40);
		...
		$hora = floor($total[$dia_anterior]); 
		$minutos = number_format(($total[$dia_anterior] - $hora)*60,0);
		if($minutos==60)
		{
			$minutos=0;
			$hora+=1;
...
<?php
 ...
 function semanacompleta($fecha=null) {
	 if(!$fecha || $fecha==null) $fecha=date('Y-m-d');
	 $semana=array();
