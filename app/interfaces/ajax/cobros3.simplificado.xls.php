...
class HTMLtoXLS {
	...
	public function echoHeader($tablaheader) {
		...
		foreach ($tablaheader as $filaheader) {
			echo '<tr><td>' . implode('</td><td>', $filaheader) . '</td></tr>';
		}
		echo '</table>';
		echo '<table border="1">';
	}
...
if ($cobranzapermitido || $mostrar_tarifa_al_profesional) {
	$cabecera[$col_tarifa_hh] = __('Tarifa HH');
	$cabecera[$col_valor_trabajo] = __('Valor Trabajo');
}
...
while ($trabajo = mysql_fetch_array($resptrabajo)) {
	...
	if ($ColumnaIdYCodigoAsuntoAExcelRevisarHoras) {
		$fila[$col_id_trabajo] = $trabajo['id_trabajo'];
	}
	...
	if ($UsoActividades) {
		$fila[$col_actividad] = $trabajo[glosa_actividad];
	}
	...
	if ($trabajo['cobrable'] == 0) {
		$duracion_cobrada = '00:00';
	}
