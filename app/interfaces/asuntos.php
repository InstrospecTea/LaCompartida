...
<script type="text/javascript">
       ...
       jQuery(document).ready(function() {
        jQuery("#agregar_asunto").click(function() {
           ...
           nuevaVentana('Agregar_Asunto',730,600,'agregar_asunto.php?codigo_cliente='+CODCLIENTE+'&popup=1&motivo=agregar_proyecto'); 
        });
    });
...
//Este query es mejorable, se podr�a sacar horas_no_cobradas y horas_trabajadas, pero ya no se podr�a ordenar por estos campos.
	...
	if (( ( method_exists('Conf', 'GetConf') && Conf::GetConf($sesion, 'CodigoSecundario') ) || ( method_exists('Conf', 'CodigoSecundario') && Conf::CodigoSecundario() ))) {
		$b->AgregarEncabezado("codigo_asunto_secundario", __('C�digo'), "align=left");
	} else {
		$b->AgregarEncabezado("codigo_asunto", __('C�digo'), "align=left");
	}
	$b->AgregarEncabezado("cliente.glosa_cliente", __('Cliente'), "align=left");
	$b->AgregarEncabezado("glosa_asunto", __('Asunto'), "align=left");
	#		$b->AgregarEncabezado("descripcion_asunto","Descripci�n","align=left");
	...
	$b->AgregarEncabezado("a1.fecha_creacion", __('Fecha de creaci�n"'));
...
function Opciones(& $fila) {
	...
	if (( ( method_exists('Conf', 'GetConf') && Conf::GetConf($sesion, 'UsaDisenoNuevo') ) || ( method_exists('Conf', 'UsaDisenoNuevo') && Conf::UsaDisenoNuevo() ))) {
		return "<a target='_parent' href=agregar_asunto.php?id_asunto=$id_asunto><img src='" . Conf::ImgDir() . "/editar_on.gif' border=0 title=Editar actividad></a>"
				. "<a href='javascript:void(0);' onclick=\"if  (confirm('�" . __('Est� seguro de eliminar el') . " " . __('asunto') . "?'))EliminaAsunto('" . $from . "'," . $id_asunto . ");\" ><img src='" . Conf::ImgDir() . "/cruz_roja_nuevo.gif' border=0 alt='Eliminar' /></a>";
	} else {
		return "<a target='_parent' href=agregar_asunto.php?id_asunto=$id_asunto><img src='" . Conf::ImgDir() . "/editar_on.gif' border=0 title=Editar actividad></a>"
				. "<a href='javascript:void(0);' onclick=\"if  (confirm('�" . __('Est� seguro de eliminar el') . " " . __('asunto') . "?'))EliminaAsunto('" . $from . "'," . $id_asunto . ");\" ><img src='" . Conf::ImgDir() . "/cruz_roja.gif' border=0 alt='Eliminar' /></a>";
	}
}
