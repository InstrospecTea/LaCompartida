<?php
	...
	if( $opc == 'guardar' || $opc == 'imprimir_factura' ) {
		...
		$factura_pdf_datos = new FacturaPdfDatos($sesion);
                ...
                if ($opc == 'guardar') echo '<div style="color:green;">Plantilla Guardada</div>';
        
		if( $opc == 'imprimir_factura' ) 	$factura_pdf_datos->generarFacturaPDF( $id_factura );
        ...
        while( $row = mysql_fetch_assoc($resp) ) 
	{
		...
		echo "<li  class='st1cell' id='glosa_".$row['id_dato']."' rel=''>".__($row['glosa_tipo_dato'])."</li>";
		echo "<li class='nd2cell'><input rel='".  $row['id_factura_pdf_datos_categoria']  ."' type=\"checkbox\" class=\"fac_activo\" name=\"fac_activo_".$row['id_dato']."\" id=\"fac_activo_".$row['id_dato']."\"  value=\"0\" disabled='disabled' />";
		...
		echo "</ul>";
                ...
                echo "<ul  class='cat_".  $row['id_factura_pdf_datos_categoria']  ."' rel='".$celda."' id=\"fila_".$row['id_dato']."\">";
		echo "<li   class='st1cell'  id='glosa_".$row['id_dato']."' rel='". $fila[$row['codigo_tipo_dato']] ."'>".__($row['glosa_tipo_dato'])."</li>";
		...
		echo "<li class='th7cell'><select name=\"fac_font_".$row['id_dato']."\" id=\"fac_font_".$row['id_dato']."\" class=\"facfont\" rel=\"".$row['id_dato']."\">
		...
		echo "<li class='th8cell'><select name=\"fac_style_".$row['id_dato']."\" id=\"fac_style_".$row['id_dato']."\" class=\"facstyle\" rel=\"".$row['id_dato']."\">
		...
		echo "<li class='th9cell'><select name=\"fac_mayuscula_".$row['id_dato']."\" id=\"fac_mayuscula_".$row['id_dato']."\" class=\"facmayus\" rel=\"".$row['id_dato']."\">
		...
		echo "<li class='th10cell'><input type=\"text\" size=\"3\" name=\"fac_tamano_".$row['id_dato']."\" id=\"fac_tamano_".$row['id_dato']."\" class=\"fontsize\" rel=\"".$row['id_dato']."\" value=\"".$row['tamano']."\" /></li>";
		echo "</ul>";
            endif;
	}
