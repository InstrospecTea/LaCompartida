...
<<<<<<< HEAD
		...
		if ( Conf::GetConf($sesion,'ClienteReferencia') && ( empty($id_cliente_referencia) || $id_cliente_referencia == '-1' ) ){
	...
	<table width="90%" cellspacing="3" cellpadding="3">
		...
		<tr class="controls controls-row ">
			<td class="ar"  width="200">
				<div class="span2">
					<?php echo  __('Codigo') ; if ($validaciones_segun_config); echo $obligatorio ?>
				</div >
			</td>
			...
			if(!Conf::GetConf($sesion, 'EncargadoSecundario')){
				...
				if(Conf::GetConf($sesion, 'AtacheSecundarioSoloAsunto')==0) {
					...
					echo '<tr class="controls controls-row">';
						echo '<td class="ar">';
				 			echo '<div class="span2">'. __('Usuario encargado') .'</div>';
								if ($validaciones_segun_config){
									echo $obligatorio;
								}
						echo '</td>';
		...
		<tr class="controls controls-row ">
			...
			<td class="al">
				<div class="span4">	<label for  class="activo">	<input type='checkbox' name='activo' id="activo" value='1' <?php echo  $cliente->fields['activo'] == 1 ? 'checked="checked"' : !$id_cliente ? 'checked="checked"' : ''  ?>/>
					&nbsp;<?php echo  __('Los clientes inactivos no aparecen en los listados.') ?></label>
				</div>
			</td>
		</tr>
...
<<<<<<< HEAD
	<table width='100%' cellspacing="0" cellpadding="0" style="<?php if (Conf::GetConf($sesion, 'AlertaCliente') )  { echo ''; } else { echo 'display:none;'; }	?>">
		<tr>
