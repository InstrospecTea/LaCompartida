...
$query ="SELECT 
					...
					$where_area
					$where_categoria
					GROUP BY trabajo.id_usuario, cliente.codigo_cliente, asunto.codigo_asunto, grupo_cliente.id_grupo_cliente
					$orderby";
#echo $query;
	...
	else if($ver == 'cliente')
	{
		...
		while($row = mysql_fetch_array($resp))
		{
				...
				$tr_usuario = "<tr><td class=usuario_nombre style='width:150px'><a href='javascript:void(0)' onclick=\"DetalleUsuario(this.form,".$row[id_usuario].")\">".$row[$letra_profesional]."</a></td>
											<td class=usuario_hr style='width:50px'><a href='javascript:void(0)' onclick=\"window.self.location.href= 'horas.php?id_usuario=".$row[id_usuario]."&fecha_ini=$fecha_ini&fecha_fin=$fecha_fin&from=asunto&codigo_cliente=".$codigo_cliente_actual."&codigo_asunto=".$id_asunto_actual."';\">".$glosa_show."</a></td></tr>\n";
				$tr_usuarios .=$tr_usuario;
