...
.inv{background-color:#42A62B;color:#FFF}
.inv a{color:#FFF}
table.res th, table.res td{padding:2px}
table.res tr{vertical-align:top}
tr.e{background-color:#EFE}
tr.o{background-color:#EEE}
tr.h{background-color:#42A62B}
tr.s{background-color:#FFF}
...
<form method="post" name="DF" enctype="multipart/form-data">
...
<b><?php echo $out_message;


if(isset($_POST['GoSQL']) && $_POST['GoSQL']=='Go') {
	echo '<script>';
	echo 'jQuery(".inv").append("| <a href=\"?qry='.base64_encode($SQLq).'\">Link a esta query</a>");';
	echo 'after_load()';
	
	if(isset($_GET['qry'])) echo "jQuery('#q').text(".base64_decode($_GET['qry']).");";
