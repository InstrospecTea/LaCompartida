...
$db_updater = new DatabaseUpdater(
		'c85ef9997e6a30032a765a20ee69630b', $connection_params
);
...
foreach ($arreglo as $sitio) {
	...
	if ($sitio['backup'] != 1) {
		...
		if ($file_exists[Key] == $filebkp) {
			...
			if ($ret) {
				$errores[] = loguear("error copiando a S3, retornado: $ret\noutput: " . implode("\n", $out));
			}
		...
		} elseif (isset($errores['espacio_base'])) {
