...
function EnviarMail($conf, $subject, $body) {
	...
	try {
		...
		$mail->SetFrom($conf['from'], 'Mailer Backups TT APP1');
...
foreach ($temps as $temp) {
	if (file_exists($temp) && !unlink($temp)) {
		...
		$aciertos[] = loguear("éxito al borrar temporal antiguo $temp");
	}
}
        ...
        foreach ($conf->dbs as $sitio) {
                ...
                if (!file_exists($dir)) {
                        ...
                        if (!mkdir($dir, 0777, true)) {
                                $errores[] = loguear("error al crear directorio del cliente: $dir");
                                continue;
                        } 
                } else {
                        $aciertos[] = loguear("Ya existe el directorio $dir por lo tanto no lo creare");
                }
            $dirpath=strval($conf->dir_temp);
                $truepath=realpath('../../tmpbackups/');

                if (!file_exists($conf->dir_temp) ) {
                        loguear("creando directorio temporal {$conf->dir_temp}");
                        $errores[] = loguear("Respaldando a $dominio no encontramos el directorio $dirpath. La funcion is_dir() arroja ".id_dir($dirpath)." y el filtro realpath arroja ".is_dir($truepath).". ");
                        if (!mkdir($conf->dir_temp, 0777, true)) {
                                $errores[] = loguear("error al crear directorio {$conf->dir_temp}. Lo raro es que debia haber existido el directorio");
                                continue;
                        }
                } else {
                                $aciertos[] = loguear("Ya existe el directorio $conf->dir_temp por lo tanto no lo crearemos");
                ...
                $aciertos[]=loguear("quedan $espacio GB libres en $dir (más que suficiente)");
                }
                ...
                exec("(mysqldump --add-drop-table -h{$conf->host} -u{$conf->login} -p{$conf->password} $db > $path) 2>&1", $out, $ret);
                        ...
                        $aciertos[] = loguear("Éxito generando dump para $db. ");
                        }
                ...
                if ($ret) {
                        ...
                        $aciertos[] = loguear("Éxito comprimiendo dump para $db");
                }

                if (file_exists($path) && !unlink($path)) {
                        $errores[] = loguear("error al borrar $path");
                } else {
                        $aciertos[] = loguear("Éxito borrando dump temporal en $path");
        }
