...
class PhpConsole {
	...
	public static function start($handleErrors = true, $handleExceptions = true, $sourceBasePath = null,$errorlevel=1) {
		if(!self::$instance) {
			if($errorlevel) error_reporting($errorlevel); 
	...
	public static function debug($message, $tags = 'debug') {
		if(self::$instance) {
			$event = new PhpConsoleEvent();
			$event->message = utf8_encode($message);
			$event->tags = $tags;
			$event->type = $tags;
