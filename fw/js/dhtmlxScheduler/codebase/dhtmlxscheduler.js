...
*/
dhtmlx=function(B){
	for(var A in B){dhtmlx[A]=B[A]}return dhtmlx
	};
	...
	window[A]=function(G)
		{
			if(G&&typeof G=="object"&&!G.tagName&&!(G instanceof Array))
				{
					var F=B.apply(this,(D._init?D._init(G):arguments));for(var E in dhtmlx){if(D[E]){this[D[E]](dhtmlx[E])}}for(var E in G){if(D[E]){this[D[E]](G[E])}else{if(E.indexOf("on")==0){this.attachEvent(E,G[E])}}}
				}
			else
				{
...
function getOffsetSum(A){
	var C=0,B=0;while(A){C=C+parseInt(A.offsetTop);B=B+parseInt(A.offsetLeft);A=A.offsetParent}return{top:C,left:B}
	}
...
function getOffsetRect(D){
	...
	var E=window.pageXOffset||B.scrollLeft||H.scrollLeft;var F=B.clientTop||H.clientTop||0;
	var I=B.clientLeft||H.clientLeft||0;
	var J=G.top+A-F;var C=G.left+E-I;
	return{top:Math.round(J),left:Math.round(C)}
}

function getOffset(A){
	if(A.getBoundingClientRect&&!_isChrome){return getOffsetRect(A)}else{return getOffsetSum(A)}
	}
	
function convertStringToBoolean(A){
	if(typeof (A)=="string"){A=A.toLowerCase()}switch(A){case"1":case"true":case"yes":case"y":case 1:case true:return true;break;default:return false}
	}
	
function getUrlSymbol(A){
	if(A.indexOf("?")!=-1){return"&"}else{return"?"}
	}
...
dhtmlDragAndDropObject.prototype.addDraggableItem=function(A,B){A.onmousedown=this.preCreateDragCopy;A.dragStarter=B;this.addDragLanding(A,B)};
...
dhtmlDragAndDropObject.prototype.callDrag=function(C){if(!C){C=window.event}dragger=window.dhtmlDragAndDrop;if((new Date()).valueOf()-dragger.downtime<100){return }if((C.button==0)&&(_isIE)){return dragger.stopDrag()}if(!dragger.dragNode&&dragger.waitDrag)
	{
		dragger.dragNode=dragger.dragStartObject._createDragNode(dragger.dragStartNode,C);if(!dragger.dragNode){return dragger.stopDrag()}dragger.dragNode.onselectstart=function(){return false};
...
if(!C.srcElement){var D=C.target}else{D=C.srcElement}dragger.checkLanding(D,C)};
...
dtmlXMLLoaderObject.prototype._filterXPath=function(B,A){var D=new Array();var A=A.replace(/[^\[]*\[\@/g,"").replace(/[\[\]\@]*/g,"");for(var C=0;C<B.length;C++){if(B[C].getAttribute(A)){D[D.length]=B[C]}}return D};

dtmlXMLLoaderObject.prototype._getAllNamedChilds=function(B,A){var E=new Array();if(_isKHTML){A=A.toUpperCase()}for(var D=0;D<B.length;D++){for(var C=0;C<B[D].childNodes.length;C++){if(_isKHTML){if(B[D].childNodes[C].tagName&&B[D].childNodes[C].tagName.toUpperCase()==A){E[E.length]=B[D].childNodes[C]}}else{if(B[D].childNodes[C].tagName==A){E[E.length]=B[D].childNodes[C]}}}}return E};
...
dtmlXMLLoaderObject.prototype.setXSLParamValue=function(B,C,D)
	{
		if(!D){D=this.xslDoc}
		if(D.responseXML){D=D.responseXML}
...
dtmlXMLLoaderObject.prototype.doXSLTransToObject=function(D,B){if(!D){D=this.xslDoc}if(D.responseXML){D=D.responseXML}if(!B){B=this.xmlDoc}if(B.responseXML){B=B.responseXML}if(!_isIE){if(!this.XSLProcessor){this.XSLProcessor=new XSLTProcessor();this.XSLProcessor.importStylesheet(D)}var A=this.XSLProcessor.transformToDocument(B)}else{var A=new ActiveXObject("Msxml2.DOMDocument.3.0");try{B.transformNodeToObject(D,A)}catch(C){A=B.transformNode(D)}}return A};
	
dtmlXMLLoaderObject.prototype.doXSLTransToString=function(C,B){var A=this.doXSLTransToObject(C,B);if(typeof (A)=="string"){return A}return this.doSerialization(A)};

dtmlXMLLoaderObject.prototype.doSerialization=function(B){if(!B){B=this.xmlDoc}if(B.responseXML){B=B.responseXML}if(!_isIE){var A=new XMLSerializer();return A.serializeToString(B)}else{return B.xml}};
...
function dataProcessor(A){
	this.serverProcessor=A;this.action_param="!nativeeditor_status";this.obj=null;this.updatedRows=[];this.autoUpdate=true;this.updateMode="cell";this._tMode="GET";this._waitMode=0;this._in_progress={};this._invalid={};this.mandatoryFields=[];this.messages=[];this.styles={updated:"font-weight:bold;",inserted:"font-weight:bold;",deleted:"text-decoration : line-through;",invalid:"background-color:FFE0E0;",invalid_cell:"border-bottom:2px solid red;",error:"color:red;",clear:"font-weight:normal;text-decoration:none;"};this.enableUTFencoding(true);dhtmlxEventable(this);return this}
...
dataProcessor.prototype={
	...
	setVerificator:function(B,A)
		{
			this.mandatoryFields[B]=A||(function(C){return(C!="")})
		},
		...
		dhtmlxEvent(window,"resize",function()
			{
				...
				scheduler._resize_timer=window.setTimeout(function(){if(scheduler.callEvent("onSchedulerResize",[])){scheduler.update_view()}},100)
			}
		);
		this.set_sizes();
...
scheduler.set_sizes=function()
	{
		...
		var F=this._els.dhx_cal_navline[0].offsetHeight;if(F>0){this.xy.nav_height=F}var C=this.xy.scale_height+this.xy.nav_height+(this._quirks?-2:0);
		this.set_xy(this._els.dhx_cal_data[0],B,D-(C+2),0,C+2)
	};

scheduler.set_xy=function(D,B,C,A,E){D.style.width=Math.max(0,B)+"px";D.style.height=Math.max(0,C)+"px";if(arguments.length>3){D.style.left=A+"px";D.style.top=E+"px"}};

scheduler.get_elements=function(){var D=this._obj.getElementsByTagName("DIV");for(var C=0;C<D.length;C++){var A=D[C].className;if(!this._els[A]){this._els[A]=[]}this._els[A].push(D[C]);var B=scheduler.locale.labels[D[C].getAttribute("name")||A];if(B){D[C].innerHTML=B}}};

scheduler.set_actions=function(){for(var A in this._els){if(this._click[A]){for(var B=0;B<this._els[A].length;B++){this._els[A][B].onclick=scheduler._click[A]}}}this._obj.onselectstart=function(C){return false};this._obj.onmousemove=function(C){scheduler._on_mouse_move(C||event)};this._obj.onmousedown=function(C){scheduler._on_mouse_down(C||event)};this._obj.onmouseup=function(C){scheduler._on_mouse_up(C||event)};this._obj.ondblclick=function(C){scheduler._on_dbl_click(C||event)}};
...
scheduler.addEventNow=function(F,B,C)
	{
		var D=this.config.time_step*60000;
		if(!F)
			{
				F=Math.round((new Date()).valueOf()/D)*D
			}
		var A=new Date(F);
		if(!B)
			{
		...
		this._loading=true;
		this.addEvent(A,new Date(B),this.locale.labels.new_event,this._drag_id);
		...
		if(!this._table_view)
		...
		else
			{
				...
				for(B=1;B<this._colsS.heights.length;B++)
					{
						if(this._colsS.heights[B]>E.y){break}
					}
...
scheduler._close_not_saved=function(){if(new Date().valueOf()-(scheduler._new_event||0)>500&&scheduler._edit_id){var A=scheduler.locale.labels.confirm_closing;if(!A||confirm(A)){scheduler.editStop(scheduler.config.positive_closing)}}};

scheduler._correct_shift=function(B,A){return B-=((new Date(scheduler._min_date)).getTimezoneOffset()-(new Date(B)).getTimezoneOffset())*60000*(A?-1:1)};
				...
				if(!this._drag_pos||this._drag_pos.x!=G.x||this._drag_pos.y!=G.y)
					{
						...
						if(this._drag_mode=="move")
							{
								B=this._min_date.valueOf()+(G.y*this.config.time_step+G.x*24*60)*60000+(this._table_view?this.date.time_part(H.start_date)*1000:0);
								B=this._correct_shift(B);
						...
						else
							{
								...
								if(this._table_view)
									{
										E=this._min_date.valueOf()+G.y*this.config.time_step*60000+24*60*60000
									}
								else
									{
								...
								if(this._drag_mode=="new-size")
									{
										if(E<=this._drag_start)
											{
												var D=this._table_view?24*60*60000:0;
												B=E-D;
						...
						if(this._table_view)
							{
								this.for_rendered(this._drag_id,function(J){J.className+=" dhx_in_move"})
							}
					}
				}
			else
...
scheduler._on_mouse_context=function(A,B){return this.callEvent("onContextMenu",[this._locate_event(B),A])};
		...
		A(scheduler.templates,
				{
					...
					event_date:B(C.hour_date_month),
...
scheduler.deleteEvent=function(C,A){var B=this._events[C];if(!A&&!this.callEvent("onBeforeEventDelete",[C,B])){return }if(B){delete this._events[C];this.unselect(C);this.event_updated(B)}};

scheduler.getEvent=function(A){return this._events[A]};

scheduler.setEvent=function(B,A){this._events[B]=A};

scheduler.for_rendered=function(C,B){for(var A=this._rendered.length-1;A>=0;A--){if(this._rendered[A].getAttribute("event_id")==C){B(this._rendered[A],A)}}};
...
scheduler.changeEventId=function(C,A)
	{
		...
		this.for_rendered(C,function(D){D.setAttribute("event_id",A)});
...
scheduler.is_visible_events=function(A){if(A.start_date<this._max_date&&this._min_date<A.end_date){return true}return false};

scheduler.is_one_day_event=function(A){var B=A.end_date.getDate()-A.start_date.getDate();return((!B||(B==1&&!A.end_date.getHours()&&!A.end_date.getMinutes()&&(A.start_date.getHours()||A.start_date.getMinutes())))&&A.start_date.getMonth()==A.end_date.getMonth()&&A.start_date.getFullYear()==A.end_date.getFullYear())};

scheduler.get_visible_events=function(){var A=[];var B=this["filter_"+this._mode];for(var C in this._events){if(this.is_visible_events(this._events[C])){if(this._table_view||this.config.multi_day||this._events[C]._timed){if(!B||B(C,this._events[C])){A.push(this._events[C])}}}}return A};

scheduler.render_view_data=function(){if(this._not_render){this._render_wait=true;return }this._render_wait=false;this.clear_view();var B=this.get_visible_events();if(this.config.multi_day&&!this._table_view){var D=[];var A=[];for(var C=0;C<B.length;C++){if(B[C]._timed){D.push(B[C])}else{A.push(B[C])}}this._table_view=true;this.render_data(A);this._table_view=false;this.render_data(D)}else{this.render_data(B)}};

scheduler.render_data=function(A,C){A=this._pre_render_events(A,C);for(var B=0;B<A.length;B++){if(this._table_view){this.render_event_bar(A[B])}else{this.render_event(A[B])}}};
...
scheduler._get_event_sday=function(A){return Math.floor((A.start_date.valueOf()-this._min_date.valueOf())/(24*60*60*1000))};

scheduler._pre_render_events_line=function(H,A){H.sort(function(K,J){return K.start_date>J.start_date?1:-1});var G=[];var I=[];for(var C=0;C<H.length;C++){var F=H[C];var D=F.start_date.getHours();var B=F.end_date.getHours();F._sday=this._get_event_sday(F);if(!G[F._sday]){G[F._sday]=[]}if(!A){F._inner=false;var E=G[F._sday];while(E.length&&E[E.length-1].end_date<=F.start_date){E.splice(E.length-1,1)}if(E.length){E[E.length-1]._inner=true}F._sorder=E.length;E.push(F);if(E.length>(E.max_count||0)){E.max_count=E.length}}if(D<this.config.first_hour||B>=this.config.last_hour){I.push(F);H[C]=F=this._copy_event(F);if(D<this.config.first_hour){F.start_date.setHours(this.config.first_hour);F.start_date.setMinutes(0)}if(B>=this.config.last_hour){F.end_date.setMinutes(0);F.end_date.setHours(this.config.last_hour)}if(F.start_date>F.end_date){H.splice(C,1);C--;continue}}}if(!A){for(var C=0;C<H.length;C++){H[C]._count=G[H[C]._sday].max_count}for(var C=0;C<I.length;C++){I[C]._count=G[I[C]._sday].max_count}}return H};

scheduler._time_order=function(A){A.sort(function(C,B){if(C.start_date.valueOf()==B.start_date.valueOf()){if(C._timed&&!B._timed){return 1}if(!C._timed&&B._timed){return -1}return 0}return C.start_date>B.start_date?1:-1})};

scheduler._pre_render_events_table=function(N,B){this._time_order(N);var D=[];var A=[[],[],[],[],[],[],[]];var L=this._colsS.heights;var G;var K=this._cols.length;for(var E=0;E<N.length;E++){var J=N[E];var H=(G||J.start_date);var F=J.end_date;if(H<this._min_date){H=this._min_date}if(F>this._max_date){F=this._max_date}var C=this.locate_holder_day(H,false,J);J._sday=C%K;var M=this.locate_holder_day(F,true,J)||K;J._eday=(M%K)||K;J._length=M-C;J._sweek=Math.floor((this._correct_shift(H.valueOf(),1)-this._min_date.valueOf())/(60*60*1000*24*K));var I=A[J._sweek];while(I.length&&I[I.length-1]._eday<=J._sday){I.splice(I.length-1,1)}if(I.length>L[J._sweek]){L[J._sweek]=I.length}J._sorder=I.length;if(J._sday+J._length<=K){G=null;D.push(J);I.push(J)}else{copy=this._copy_event(J);copy._length=K-J._sday;copy._eday=K;copy._sday=J._sday;copy._sweek=J._sweek;copy._sorder=J._sorder;copy.end_date=this.date.add(H,copy._length,"day");D.push(copy);I.push(copy);G=copy.end_date;E--;continue}}return D};

scheduler._copy_dummy=function(){this.start_date=new Date(this.start_date);this.end_date=new Date(this.end_date)};

scheduler._copy_event=function(A){this._copy_dummy.prototype=A;return new this._copy_dummy()};

scheduler._rendered=[];scheduler.clear_view=function(){for(var A=0;A<this._rendered.length;A++){var B=this._rendered[A];if(B.parentNode){B.parentNode.removeChild(B)}}this._rendered=[]};
...
scheduler.clear_event=function(A){this.for_rendered(A,function(C,B){if(C.parentNode){C.parentNode.removeChild(C)}scheduler._rendered.splice(B,1)})};
...
scheduler.render_event=function(L)
	{
		...
		var B=Math.ceil((M.clientWidth-D)/L._count);
		var E=L._sorder*B+1;
		if(!L._inner)
			{
...
scheduler._render_v_bar=function(D,M,L,N,H,B,F,E,A){var J=document.createElement("DIV");var K=this.getEvent(D);var I="dhx_cal_event";var C=scheduler.templates.event_class(K.start_date,K.end_date,K);if(C){I=I+" "+C}var G='<div event_id="'+D+'" class="'+I+'" style="position:absolute; top:'+L+"px; left:"+M+"px; width:"+(N-4)+"px; height:"+H+"px;"+(B||"")+'">';G+='<div class="dhx_header" style=" width:'+(N-6)+'px;" >&nbsp;</div>';G+='<div class="dhx_title">'+F+"</div>";G+='<div class="dhx_body" style=" width:'+(N-(this._quirks?4:14))+"px; height:"+(H-(this._quirks?20:30))+'px;">'+E+"</div>";G+='<div class="dhx_footer" style=" width:'+(N-8)+"px;"+(A?" margin-top:-1px;":"")+'" ></div></div>';J.innerHTML=G;return J.firstChild};

scheduler.locate_holder=function(A){if(this._mode=="day"){return this._els.dhx_cal_data[0].firstChild}return this._els.dhx_cal_data[0].childNodes[A]};

scheduler.locate_holder_day=function(B,C){var A=Math.floor((this._correct_shift(B,1)-this._min_date)/(60*60*24*1000));if(C&&this.date.time_part(B)){A++}return A};
...
scheduler.render_event_bar=function(H)
	{
		...
		var F=document.createElement("DIV");var E=H._timed?"dhx_cal_event_clear":"dhx_cal_event_line";
...
scheduler._locate_event=function(A){var B=null;while(A&&!B&&A.getAttribute){B=A.getAttribute("event_id");A=A.parentNode}return B};
...
scheduler.getEvents=function(E,D){var A=[];for(var B in this._events){var C=this._events[B];if(C&&C.start_date<D&&C.end_date>E){A.push(C)}}return A};

scheduler._loaded={};

scheduler._load=function(C,F){C=C||this._load_url;C+=(C.indexOf("?")==-1?"?":"&")+"timeshift="+(new Date()).getTimezoneOffset();if(this.config.prevent_cache){C+="&uid="+this.uid()}var E;F=F||this._date;if(this._load_mode){var B=this.templates.load_format;F=this.date[this._load_mode+"_start"](new Date(F.valueOf()));while(F>this._min_date){F=this.date.add(F,-1,this._load_mode)}E=F;var D=true;while(E<this._max_date){E=this.date.add(E,1,this._load_mode);if(this._loaded[B(F)]&&D){F=this.date.add(F,1,this._load_mode)}else{D=false}}var A=E;do{E=A;A=this.date.add(E,-1,this._load_mode)}while(A>F&&this._loaded[B(A)]);if(E<=F){return false}dhtmlxAjax.get(C+"&from="+B(F)+"&to="+B(E),function(G){scheduler.on_load(G)});while(F<E){this._loaded[B(F)]=true;F=this.date.add(F,1,this._load_mode)}}else{dhtmlxAjax.get(C,function(G){scheduler.on_load(G)})}this.callEvent("onXLS",[]);return true};

scheduler.on_load=function(A){this._loading=true;if(this._process){var B=this[this._process].parse(A.xmlDoc.responseText)}else{var B=this._magic_parser(A)}this._not_render=true;for(var C=0;C<B.length;C++){if(!this.callEvent("onEventLoading",[B[C]])){continue}this.addEvent(B[C])}this._not_render=false;if(this._render_wait){this.render_view_data()}if(this._after_call){this._after_call()}this._after_call=null;this._loading=false;this.callEvent("onXLE",[])};

scheduler.json={};

scheduler.json.parse=function(data){if(typeof data=="string"){eval("scheduler._temp = "+data+";");data=scheduler._temp}var evs=[];for(var i=0;i<data.length;i++){data[i].start_date=scheduler.templates.xml_date(data[i].start_date);data[i].end_date=scheduler.templates.xml_date(data[i].end_date);evs.push(data[i])}return evs};

scheduler.parse=function(B,A){this._process=A;this.on_load({xmlDoc:{responseText:B}})};
...
scheduler.setLoadMode=function(A){if(A=="all"){A=""}this._load_mode=A};

scheduler.refresh=function(A){alert("not implemented")};

scheduler.serverList=function(A){return this.serverList[A]=(this.serverList[A]||[])};

scheduler._magic_parser=function(H){if(!H.getXMLTopNode){var B=H.xmlDoc.responseText;H=new dtmlXMLLoaderObject(function(){});H.loadXMLString(B)}var E=H.getXMLTopNode("data");if(E.tagName!="data"){return[]}var A=H.doXPath("//coll_options");for(var D=0;D<A.length;D++){var G=A[D].getAttribute("for");var F=this.serverList[G];if(!F){continue}F.splice(0,F.length);var J=H.doXPath("//item",A[D]);for(var C=0;C<J.length;C++){F.push({key:J[C].getAttribute("value"),label:J[C].getAttribute("label")})}}if(A.length){scheduler.callEvent("onOptionsLoad",[])}var I=[];var E=H.doXPath("//event");for(var D=0;D<E.length;D++){I[D]=this.xmlNodeToJSON(E[D]);I[D].text=I[D].text||I[D]._tagvalue;I[D].start_date=this.templates.xml_date(I[D].start_date);I[D].end_date=this.templates.xml_date(I[D].end_date)}return I};

scheduler.xmlNodeToJSON=function(C){var B={};for(var A=0;A<C.attributes.length;A++){B[C.attributes[A].name]=C.attributes[A].value}for(var A=0;A<C.childNodes.length;A++){var D=C.childNodes[A];if(D.nodeType==1){B[D.tagName]=D.firstChild?D.firstChild.nodeValue:""}}if(!B.text){B.text=C.firstChild?C.firstChild.nodeValue:""}return B};

scheduler.attachEvent("onXLS",function(){if(this.config.show_loading===true){var A;A=this.config.show_loading=document.createElement("DIV");A.className="dhx_loading";A.style.left=Math.round((this._x-128)/2)+"px";A.style.top=Math.round((this._y-15)/2)+"px";this._obj.appendChild(A)}});scheduler.attachEvent("onXLE",function(){var A;if(A=this.config.show_loading){if(typeof A=="object"){this._obj.removeChild(A);this.config.show_loading=true}}});scheduler.ical={parse:function(H){var E=H.match(RegExp(this.c_start+"[^\f]*"+this.c_end,""));if(!E.length){return }E[0]=E[0].replace(/[\r\n]+(?=[a-z \t])/g," ");E[0]=E[0].replace(/\;[^:]*/g,"");var B=[];var D;var C=RegExp("(?:"+this.e_start+")([^\f]*?)(?:"+this.e_end+")","g");while(D=C.exec(E)){var F={};var G;var A=/[^\r\n]+[\r\n]+/g;while(G=A.exec(D[1])){this.parse_param(G.toString(),F)}B.push(F)}return B},parse_param:function(E,C){var D=E.indexOf(":");if(D==-1){return }var A=E.substr(0,D).toLowerCase();var B=E.substr(D+1).replace(/\\\,/g,",").replace(/[\r\n]+$/,"");if(A=="summary"){A="text"}else{if(A=="dtstart"){A="start_date";B=this.parse_date(B,0,0)}else{if(A=="dtend"){A="end_date";if(C.start_date&&C.start_date.getHours()==0){B=this.parse_date(B,24,0)}else{B=this.parse_date(B,23,59)}}}}C[A]=B},parse_date:function(G,F,D){var E=G.split("T");if(E[1]){F=E[1].substr(0,2);D=E[1].substr(2,2)}var C=E[0].substr(0,4);var B=parseInt(E[0].substr(4,2),10)-1;var A=E[0].substr(6,2);return new Date(C,B,A,F,D)},c_start:"BEGIN:VCALENDAR",e_start:"BEGIN:VEVENT",e_end:"END:VEVENT",c_end:"END:VCALENDAR"};

scheduler.form_blocks={textarea:{render:function(B){var A=(B.height||"130")+"px";return"<div class='dhx_cal_ltext' style='height:"+A+";'><textarea></textarea></div>"},set_value:function(B,C,A){B.firstChild.value=C||""},get_value:function(B,A){return B.firstChild.value},focus:function(B){var A=B.firstChild;A.select();A.focus()}},select:{render:function(D){var A=(D.height||"23")+"px";var C="<div class='dhx_cal_ltext' style='height:"+A+";'><select style='width:552px;'>";for(var B=0;B<D.options.length;B++){C+="<option value='"+D.options[B].key+"'>"+D.options[B].label+"</option>"}C+="</select></div>";return C},set_value:function(B,C,A){if(typeof C=="undefined"){C=(B.firstChild.options[0]||{}).value}B.firstChild.value=C||""},get_value:function(B,A){return B.firstChild.value},focus:function(B){var A=B.firstChild;A.select();A.focus()}},time:{render:function(){var A=scheduler.config;var E=this.date.date_part(new Date());var D=24*60,G=0;if(scheduler.config.limit_time_select){D=60*A.last_hour+1;G=60*A.first_hour;E.setHours(A.first_hour)}var C="<select>";for(var B=G;B<D;B+=this.config.time_step*1){var F=this.templates.time_picker(E);C+="<option value='"+B+"'>"+F+"</option>";E=this.date.add(E,this.config.time_step,"minute")}C+="</select> <select>";for(var B=1;B<32;B++){C+="<option value='"+B+"'>"+B+"</option>"}C+="</select> <select>";for(var B=0;B<12;B++){C+="<option value='"+B+"'>"+this.locale.date.month_full[B]+"</option>"}C+="</select> <select>";E=E.getFullYear()-5;for(var B=0;B<10;B++){C+="<option value='"+(E+B)+"'>"+(E+B)+"</option>"}C+="</select> ";return"<div style='height:30px; padding-top:0px; font-size:inherit;' class='dhx_cal_lsection dhx_section_time'>"+C+"<span style='font-weight:normal; font-size:10pt;'> &nbsp;&ndash;&nbsp; </span>"+C+"</div>"},set_value:function(D,E,C){function A(G,F,H){G[F+0].value=Math.round((H.getHours()*60+H.getMinutes())/scheduler.config.time_step)*scheduler.config.time_step;G[F+1].value=H.getDate();G[F+2].value=H.getMonth();G[F+3].value=H.getFullYear()}var B=D.getElementsByTagName("select");A(B,0,C.start_date);A(B,4,C.end_date)},get_value:function(B,A){s=B.getElementsByTagName("select");A.start_date=new Date(s[3].value,s[2].value,s[1].value,0,s[0].value);A.end_date=new Date(s[7].value,s[6].value,s[5].value,0,s[4].value);if(A.end_date<=A.start_date){A.end_date=scheduler.date.add(A.start_date,scheduler.config.time_step,"minute")}},focus:function(A){A.getElementsByTagName("select")[0].focus()}}};

scheduler.showCover=function(A){this.show_cover();if(A){A.style.display="block";var B=getOffset(this._obj);A.style.top=Math.round(B.top+(this._obj.offsetHeight-A.offsetHeight)/2)+"px";A.style.left=Math.round(B.left+(this._obj.offsetWidth-A.offsetWidth)/2)+"px"}};

scheduler.showLightbox=function(B){if(!B){return }if(!this.callEvent("onBeforeLightbox",[B])){return }var A=this._get_lightbox();this.showCover(A);this._fill_lightbox(B,A)};

scheduler._fill_lightbox=function(H,E){var D=this.getEvent(H);var B=E.getElementsByTagName("span");B[1].innerHTML=this.templates.event_header(D.start_date,D.end_date,D);B[2].innerHTML=this.templates.event_bar_text(D.start_date,D.end_date,D);var F=this.config.lightbox.sections;for(var A=0;A<F.length;A++){var C=document.getElementById(F[A].id).nextSibling;var G=this.form_blocks[F[A].type];G.set_value.call(this,C,D[F[A].map_to],D);if(F[A].focus){G.focus.call(this,C)}}scheduler._lightbox_id=H};

scheduler._lightbox_out=function(D){var E=this.config.lightbox.sections;for(var B=0;B<E.length;B++){var C=document.getElementById(E[B].id).nextSibling;var F=this.form_blocks[E[B].type];var A=F.get_value.call(this,C,D);if(E[B].map_to!="auto"){D[E[B].map_to]=A}}return D};

scheduler._empty_lightbox=function(){var C=scheduler._lightbox_id;var B=this.getEvent(C);var A=this._get_lightbox();this._lightbox_out(B);B._timed=this.is_one_day_event(B);this.setEvent(B.id,B);this._edit_stop_event(B,true);this.render_view_data()};

scheduler.hide_lightbox=function(A){this.hideCover(this._get_lightbox());this._lightbox_id=null;this.callEvent("onAfterLightbox",[])};

scheduler.hideCover=function(A){if(A){A.style.display="none"}this.hide_cover()};

scheduler.hide_cover=function(){if(this._cover){this._cover.parentNode.removeChild(this._cover)}this._cover=null};

scheduler.show_cover=function(){this._cover=document.createElement("DIV");this._cover.className="dhx_cal_cover";document.body.appendChild(this._cover)};

scheduler.save_lightbox=function(){if(this.checkEvent("onEventSave")&&!this.callEvent("onEventSave",[this._lightbox_id,this._lightbox_out({})])){return }this._empty_lightbox();this.hide_lightbox()};

scheduler.cancel_lightbox=function(){this.callEvent("onEventCancel",[this._lightbox_id]);this._edit_stop_event(scheduler.getEvent(this._lightbox_id),false);this.hide_lightbox()};

scheduler._init_lightbox_events=function(){this._get_lightbox().onclick=function(C){var E=C?C.target:event.srcElement;if(!E.className){E=E.previousSibling}if(E&&E.className){switch(E.className){case"dhx_save_btn":scheduler.save_lightbox();break;case"dhx_delete_btn":var F=scheduler.locale.labels.confirm_deleting;if(!F||confirm(F)){scheduler.deleteEvent(scheduler._lightbox_id);scheduler._new_event=null;scheduler.hide_lightbox()}break;case"dhx_cancel_btn":scheduler.cancel_lightbox();break;default:if(E.className.indexOf("dhx_custom_button_")!=-1){var A=E.parentNode.getAttribute("index");var D=scheduler.form_blocks[scheduler.config.lightbox.sections[A].type];var B=E.parentNode.parentNode;D.button_click(A,E,B,B.nextSibling)}}}};this._get_lightbox().onkeypress=function(A){switch((A||event).keyCode){case scheduler.keys.edit_save:if((A||event).shiftKey){return }scheduler.save_lightbox();break;case scheduler.keys.edit_cancel:scheduler.cancel_lightbox();break}}};
...
scheduler.setUserData=function(C,A,B){this.getEvent(C)[A]=B};

scheduler.getUserData=function(B,A){return this.getEvent(B)[A]};
...
scheduler.validId=function(A){return true};
